...
def create_app(settings:PlatformSettings|None=None)->FastAPI:
    ...
    async def daily_microstructure(session_date:date,symbol:str=Query("QQQ")):
        # A full daily report is generated by a bounded background job.  The
        # web process must not materialize an entire raw-chain day on demand.
        day=datetime.combine(session_date,time.min,tzinfo=timezone.utc)
        report=await container.repository.daily_microstructure_report(symbol,day)
        if report is not None:return report
        return {"date":session_date.isoformat(),"symbol":symbol.upper(),"status":"PENDING_BACKGROUND_BUILD",
            "total_levels_count":0,"call_levels_count":0,"put_levels_count":0,"dense_levels_count":0,
            "key_levels":[],"paradigm_shift_levels":[]}
