...
GAMMA_DYNAMICS_V3_GREEKS = ("gamma", "speed", "color", "charm", "delta")
...
class GammaDynamicsEarlyMove:
    ...
    def __init__(self, cadence_seconds: int = 10, market_timezone: str = "America/New_York"):
        ...
        self._color_distribution: dict[str,deque[float]]=defaultdict(lambda:deque(maxlen=6000))
    ...
    def _waiting(self,symbol:str,history_points:int,reason:str,metrics:dict[str,Any]|None=None) -> GammaDynamics:
        return GammaDynamics(
            ...
            ideal_ranges={
                ...
                "speed":"Direction-setting gamma acceleration", "color":"Directional time-pressure transition",
    ...
    def calculate(self,contracts: Sequence[dict[str,Any]], spot: float, timestamp: datetime, source_symbol: str) -> GammaDynamics:
        ...
        eligible=[]
        for row in clean:
            gamma=float(row.get("gamma") or 0.0);speed=float(row.get("speed") or 0.0);color=float(row.get("color") or 0.0);charm=float(row.get("charm") or 0.0);delta=float(row.get("delta") or 0.0)
            ...
            observation={"timestamp":timestamp,"gamma":gamma,"speed":speed,"color":color,"charm":charm,"delta":delta,"iv":iv,"spot":spot,"volume":float(row.get("volume") or 0.0),"years":years}
            self._contracts[key].append(observation)
            self._gamma_distribution[symbol].append(abs(gamma));self._speed_distribution[symbol].append(abs(speed));self._color_distribution[symbol].append(abs(color));self._charm_distribution[symbol].append(abs(charm))
        ...
        gamma_p90=max(self._p90(self._gamma_distribution[symbol]),1e-12);speed_p90=max(self._p90(self._speed_distribution[symbol]),1e-12);color_p90=max(self._p90(self._color_distribution[symbol]),1e-12);charm_p90=max(self._p90(self._charm_distribution[symbol]),1e-12)
        ...
        for row,key,current in eligible:
            ...
            if not prior_5m or not prior_1m or not spot_5m or not spot_1m:
                continue
            gamma=float(current["gamma"]);speed=float(current["speed"]);color=float(current["color"]);charm=float(current["charm"]);delta=float(current["delta"]);years=float(current["years"]);iv=float(current["iv"]);strike=float(row.get("strike") or 0.0)
            ...
            gamma_term=min(abs(gamma)/gamma_p90,3.0);speed_term=min(abs(speed)/speed_p90,3.0);color_term=min(abs(color)/color_p90,3.0);charm_term=min(abs(charm)/charm_p90,3.0);delta_term=abs(delta)/.70
            score=gamma_term*(1.0+.45*speed_term)*(1.0+.25*color_term)*(1.0+.20*charm_term)*(0.70+.30*delta_term)
            ...
            directional_color_flip=(direction_sign*float(prior_5m["color"])<0 and direction_sign*color>0)
            ...
            checks={
                ...
                "color_transition":direction!=Direction.NEUTRAL and directional_color_flip,
        ...
        normalized={"gamma":self._clip(inputs["gamma"]/gamma_p90),"speed":self._clip(inputs["speed"]/speed_p90),"color":self._clip(inputs["color"]/color_p90),"charm":self._clip(inputs["charm"]/charm_p90),"delta":self._clip(inputs["delta"]/.70)}
        metrics={"model_version":"GAMMA_3_EARLY_MOVE","chain_available":float(bool(clean)),"eligible_contracts":len(eligible),"selected_strike":float(row.get("strike") or 0.0),"spot":spot,"net_gex_strike":detail["strike_gex"],"gamma_p90_rolling":gamma_p90,"speed_p90_rolling":speed_p90,"color_p90_rolling":color_p90,"charm_p90_rolling":charm_p90,"score":score,"score_5m_ago":detail["prior_score"],"dscore":detail["dscore"],"directional_speed_acceleration":detail["directional_speed_accel"],"charm_acceleration_1m":detail["charm_acceleration"],"realized_range_5m":range_5m,"realized_vol_5m":realized_vol_5m,"implied_volatility":float(current["iv"]),"median_contract_volume":median_contract_volume,"option_volume_participation":detail["option_volume_participation"],"hedge_impulse":detail["hedge_impulse"],"spot_momentum_1m":spot-float(spot_1m["spot"]),"target_points":1.25,"target_horizon_minutes":5.0,"evaluation_cadence_seconds":self.cadence_seconds,"expected_move_label":"1.25-POINT TARGET · CALIBRATION REQUIRED"}
        checks={"baseline":True,**checks}
        qualified=bool(passed)
        if qualified:
            explanation=(f"Quiet 5-minute coil with {direction.value.lower()} Speed acceleration, directional Color transition, accelerating negative Charm, and ${abs(detail['strike_gex'])/1e9:.2f}B signed strike GEX. Gamma 3.0 is tracking a 1.25-point QQQ target over five minutes.")
        else:
        ...
        result=GammaDynamics(decision=direction if qualified else Direction.NEUTRAL,qualified=qualified,source_symbol=symbol,intensity=min(1.0,score/8.0),pressure=(1 if direction==Direction.UP else -1 if direction==Direction.DOWN else 0)*min(1.0,detail["hedge_impulse"]/2.0),history_points=baseline_points,intensity_threshold=1.0,inputs=inputs,percentiles={"gamma":min(1.0,abs(inputs["gamma"])/gamma_p90),"speed":min(1.0,abs(inputs["speed"])/speed_p90),"color":min(1.0,abs(inputs["color"])/color_p90),"charm":min(1.0,abs(inputs["charm"])/charm_p90),"delta":min(1.0,abs(inputs["delta"])/.70)},normalized=normalized,contributions={"gamma":detail["gamma_term"],"speed":detail["speed_term"],"color":detail["color_term"],"charm":detail["charm_term"],"delta":detail["delta_term"]},ideal_ranges={"gamma":"Rolling p90 curvature participation","speed":"Directional five-minute acceleration","color":"Must cross into the Speed direction over five minutes","charm":"≤ -0.03 and falling by at least 0.01 in one minute","delta":"Liquid near-ATM |Delta| 0.30–0.70"},chain_metrics=metrics,normalized_features=normalized,squeeze_score=detail["hedge_impulse"],probability=min(.95,max(0.0,.50+.10*min(score,3)+.08*min(detail["hedge_impulse"],2))),target_price=spot+(1.25 if direction==Direction.UP else -1.25) if qualified else None,alert_checks=checks,explanation=explanation)
