...
class WallIntelligenceService:
    ...
    def observe(self,timestamp:Any,symbol:str,spot:float,metrics:dict[str,Any],regime:str,volume:float)->tuple[dict[str,Any],list[dict[str,Any]]]:
        ...
        tw=float(metrics.get("tw_gex",0));spoof=float(metrics.get("spoof_score",0));edge=float(metrics.get("edge",0));walls={};breaks=[];prior_vol=list(self.volumes);surge=float(volume)/max(sum(prior_vol)/len(prior_vol),1.) if prior_vol else 0.
        for kind,strike in levels.items():
            ...
            if direction:breaks.append({"timestamp":timestamp,"symbol":symbol,"wall_type":kind,"strike":strike,"direction":direction,"tier":reading["tier"],"spot":spot,"gex_dollar":dollar,"build_intensity":max(0.,tw)*max(0.,edge)/(spoof+.5),"edge":edge,"liquidity":float(metrics.get("liquidity_score",0)),"vix":float(metrics.get("vix",0)),"volume":float(volume),"volume_surge":surge,"delta_change":0.,"spot_change":spot-prior[0],"qqq_return":(spot-prior[0])/prior[0] if prior[0] else 0.,"retest_count":0,"path_efficiency_5m":None,"outcome":"PENDING","regime":regime,"is_point_in_time":True,"is_estimated_oi_delayed":True})
