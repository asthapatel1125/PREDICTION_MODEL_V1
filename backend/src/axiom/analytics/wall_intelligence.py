...
WALL_TYPES=("CALL_WALL","PUT_WALL","ZERO_GAMMA","SUPPORT","RESISTANCE","DELTA_WALL","ZERO_DELTA")
...
class WallIntelligenceService:
    ...
    def observe(self,timestamp:Any,symbol:str,spot:float,metrics:dict[str,Any],regime:str,volume:float,pressure_trend:float=50.0)->tuple[dict[str,Any],list[dict[str,Any]]]:
        ...
        for kind,strike in levels.items():
            raw=abs(float(estimates.get(kind,{}).get("dex",metrics.get("zero_delta_dex",0) if kind=="ZERO_DELTA" else metrics.get("delta_wall_dex",0)))) if kind in {"DELTA_WALL","ZERO_DELTA"} else abs(float(estimates.get(kind,{}).get("gex",gex.get(strike,abs(float(metrics.get("call_wall_gex",0))) if kind=="CALL_WALL" else abs(float(metrics.get("put_wall_gex",0))) if kind=="PUT_WALL" else 0.))))
            hist=list(self.history[kind]);z=self._z(raw,hist);pct=self._pct(raw,hist);dollar=raw
            # Preserve the signed per-strike exposure alongside the absolute
            # strength used for ranking.  A price level is always positive;
            # the sign for the chart must come from GEX/DEX itself.
            signed_exposure=(
                float(estimates.get(kind,{}).get("dex",metrics.get("zero_delta_dex",0) if kind=="ZERO_DELTA" else metrics.get("delta_wall_dex",0)))
                if kind in {"DELTA_WALL","ZERO_DELTA"}
        ...
        derived={**chain_imbalances(metrics),**{key:metrics.get(key,0) for key in (
            "pressure_composite_raw","speed_pct","zomma_pct","dex_pct","gex_pct","dealer_flow_pct","dealer_delta_velocity","zero_gamma_pct","effective_gex_pct",
        ...
        # This observer is fed exclusively by LiveEngine's ThetaData Options
        # Pro option-chain bar. Persist the provenance with the point so HTTP
        # backfills and WebSocket updates carry the same auditable contract.
        point={"timestamp":timestamp,"symbol":symbol,"spot":spot,"volume":float(volume),"walls":walls,"dex":float(metrics.get("dex",0)),"dex_signed_raw":float(metrics["dex_signed_raw"]) if metrics.get("dex_signed_raw") is not None else None,"gamma_exposure_raw":float(metrics["gex_raw"]) if metrics.get("gex_raw") is not None else None,"charm_exposure_raw":float(metrics["charm_exposure_raw"]) if metrics.get("charm_exposure_raw") is not None else None,"speed_exposure_raw":float(metrics["speed_exposure_raw"]) if metrics.get("speed_exposure_raw") is not None else None,"greek_exposures":metrics.get("greek_exposures") or {},"vol_hack":float(metrics.get("vol_hack",0)),"dealer_flow":float(metrics.get("dealer_flow",0)),"pos_inventory":float(metrics.get("pos_inventory",0)),"neg_inventory":float(metrics.get("neg_inventory",0)),"tw_gex":tw,"gex_density":float(metrics.get("gex_density",0)),"gex_dollar_density":float(metrics.get("gex_dollar_density",0)),"spoof_score":spoof,"edge":edge,"liquidity":float(metrics.get("liquidity_score",0)),"vix":float(metrics.get("vix",0)),"pressure_trend":max(0.0,min(100.0,float(pressure_trend))),**derived,"regime":regime,"provider":"THETADATA_OPTIONS_PRO","feed":"OPTION_SNAPSHOT_GREEKS_ALL","cadence_seconds":5,"is_point_in_time":True,"is_estimated_oi_delayed":True,"disclaimer":WALL_INTELLIGENCE_DISCLAIMER}
        return point,breaks
