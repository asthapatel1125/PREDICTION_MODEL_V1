...
class GammaDynamicsQuartet:
    ...
    def calculate(self, greeks: Greeks, history: Sequence[Greeks], source_symbol: str) -> GammaDynamics:
        ...
        aligned_up = inputs["speed"] > self.zero_tolerance and inputs["gamma"] > self.zero_tolerance
        aligned_down = inputs["speed"] < -self.zero_tolerance and inputs["gamma"] < -self.zero_tolerance
        ...
        if not warmed:
            ...
            explanation = "Gamma and Speed disagree, so signed curvature pressure is not confirmed."
            ...
            explanation = f"Gamma and Speed confirm {direction} curvature pressure while Zomma and Color show elevated Gamma sensitivity."
