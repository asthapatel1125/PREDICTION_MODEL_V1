...
function deriveGammaDynamicsV2(state,history=[]){
  ...
  const percentiles=Object.fromEntries(Object.entries(inputs).map(([name,value])=>[name,percentile(name,value)])),normalized=Object.fromEntries(Object.entries(inputs).map(([name,value])=>[name,scaled(name,value)])),intensity=.30*Math.abs(normalized.zomma)+.25*Math.abs(normalized.color)+.25*Math.abs(normalized.ultima)+.20*Math.abs(normalized.vomma),gammaActive=Math.abs(inputs.gamma)>1e-12,alignedUp=inputs.speed>1e-12&&gammaActive,alignedDown=inputs.speed< -1e-12&&gammaActive,pressureMagnitude=.45*Math.abs(normalized.speed)+.30*Math.abs(normalized.gamma)+.15*Math.abs(normalized.zomma)+.10*Math.abs(normalized.vomma),warmed=rows.length>=20,qualified=warmed&&intensity>=.65&&(alignedUp||alignedDown),decision=qualified?(alignedUp?"UP":"DOWN"):"NEUTRAL";
  const ideal_ranges={zomma:"|normalized| ≥ 0.30 · elevated IV-to-Gamma sensitivity",color:"|normalized| ≥ 0.30 · elevated time-to-Gamma sensitivity",speed:"|normalized| ≥ 0.30 and signed with call direction",gamma:"|normalized| ≥ 0.30 · active curvature base",ultima:"|normalized| ≥ 0.30 · volatility instability",vomma:"|normalized| ≥ 0.30 · volatility convexity"};
