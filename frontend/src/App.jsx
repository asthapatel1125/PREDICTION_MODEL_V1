...
function deriveGammaDynamics(state,history=[]){
  ...
  const scaled=(name,value)=>{const values=rows.map(row=>greekValue(row,name)).filter(Number.isFinite);if(!values.length)return 0;const mean=values.reduce((sum,item)=>sum+item,0)/values.length,variance=values.reduce((sum,item)=>sum+(item-mean)**2,0)/values.length,std=Math.sqrt(variance);return std<=1e-12?0:Math.max(-3,Math.min(3,(value-mean)/std))/3};
  const percentiles=Object.fromEntries(Object.entries(inputs).map(([name,value])=>[name,percentile(name,value)])),normalized=Object.fromEntries(Object.entries(inputs).map(([name,value])=>[name,scaled(name,value)])),intensity=.30*percentiles.zomma+.25*percentiles.color+.25*percentiles.ultima+.20*percentiles.vomma,gammaActive=Math.abs(inputs.gamma)>1e-12,alignedUp=inputs.speed>1e-12&&gammaActive,alignedDown=inputs.speed< -1e-12&&gammaActive,pressureMagnitude=.45*percentiles.speed+.30*percentiles.gamma+.15*percentiles.zomma+.10*percentiles.vomma,warmed=rows.length>=20,qualified=warmed&&intensity>=.65&&(alignedUp||alignedDown),decision=qualified?(alignedUp?"UP":"DOWN"):"NEUTRAL";
  const ideal_ranges={zomma:"Magnitude percentile ≥ 65% · elevated IV-to-Gamma sensitivity",color:"Magnitude percentile ≥ 65% · elevated time-to-Gamma sensitivity",speed:"Non-zero and signed with call direction",gamma:"Non-zero curvature base · ≥ 50% preferred",ultima:"Magnitude percentile ≥ 65% · volatility instability",vomma:"Magnitude percentile ≥ 65% · volatility convexity"};
  return {available:true,qualified,decision,intensity,pressure:alignedUp?pressureMagnitude:alignedDown?-pressureMagnitude:0,history_points:rows.length,intensity_threshold:.65,inputs,percentiles,normalized,ideal_ranges,explanation:!warmed?`Building a relative baseline: ${rows.length}/20 observations.`:!(alignedUp||alignedDown)?"Gamma or Speed is effectively zero, so signed curvature pressure is not confirmed.":intensity<.65?"Speed has direction, but normalized Zomma/Color/Ultima/Vomma intensity is below its rolling threshold.":`Speed indicates ${alignedUp?"upward":"downward"} curvature change while Gamma, Zomma, Color, Ultima, and Vomma confirm the curvature environment.`};
}
...
function deriveGammaDynamicsEvents(history,state,symbol){
  ...
  const events=[];let prior="NEUTRAL",window=[];rows.forEach(row=>{window=[...window,row].slice(-100);const result=deriveGammaDynamics(row,window),decision=result.qualified?result.decision:"NEUTRAL";if(decision!=="NEUTRAL"&&decision!==prior)events.push({id:numericEventId(row.timestamp,3),timestamp:row.timestamp,symbol:row.symbol??symbol,price:number(row?.supporting_indicators?.price,NaN),decision,intensity:result.intensity,pressure:result.pressure,normalized:result.normalized??{},...result.inputs});prior=decision});return events.reverse();
}

const SIX_GREEKS=["ultima","zomma","gamma","speed","color","delta"];
const GAMMA_DYNAMICS_GREEKS=["zomma","color","speed","gamma","ultima","vomma"];
...
function GammaDynamicsModule({state,history,symbol,engine}){
  ...
  return <section className={`gamma-dynamics gamma-dynamics-${tone}`} aria-live="polite"><header><div><span>GAMMA DYNAMICS · SIX-GREEK ENGINE</span><h2>{symbol} · {label}</h2><small>{engine.running?"● LIVE OPTIONS PRO":"○ ENGINE IDLE"} · relative to the latest {quartet.history_points??0} observations</small></div><div className="gamma-dynamics-score"><span>DYNAMICS INTENSITY</span><b>{pct(quartet.intensity)}</b><small>Ideal ≥ {pct(quartet.intensity_threshold??.65)}</small></div><div className="gamma-dynamics-score pressure"><span>CURVATURE PRESSURE</span><b>{number(quartet.pressure)>0?"+":""}{number(quartet.pressure).toFixed(2)}</b><small>Direction: Speed · Base: Gamma magnitude</small></div></header><div className="gamma-six-target"><span>QQQ SESSION TARGET <b>{gammaTargetPoints(state?.timestamp).toFixed(2)} PT</b></span><span>PROJECTED REACH <b>{Number.isFinite(target)?target.toFixed(4):"—"}</b></span><small>1.25 points from 9:30 AM–12:00 PM ET · 0.75 points at all other times</small></div><div className="gamma-dynamics-ideals"><span className={intensityPassed?"passed":"waiting"}><b>{intensityPassed?"PASS":"WAIT"}</b> Weighted vol/time intensity ≥ {pct(quartet.intensity_threshold??.65)}</span><span className={aligned?"passed":"waiting"}><b>{aligned?"PASS":"WAIT"}</b> Speed directional + Gamma active</span><span className={warmed?"passed":"waiting"}><b>{warmed?"PASS":"WAIT"}</b> Baseline ≥ 20 observations</span></div><div className="gamma-dynamics-grid gamma-six-grid">{GAMMA_DYNAMICS_GREEKS.map(name=>{const value=quartet.inputs?.[name],rank=quartet.percentiles?.[name]??0,scaled=number(quartet.normalized?.[name]);return <article key={name}><div><span>{name.toUpperCase()}</span><small>{metadata[name][0]}</small></div><b>{scaled>=0?"+":""}{scaled.toFixed(3)} <small>NORM</small></b><i><em style={{width:`${Math.max(2,rank*100)}%`}}/></i><p>Raw {signedGreek(value)} · {metadata[name][1]} · magnitude {pct(rank)}</p><strong>IDEAL: {quartet.ideal_ranges?.[name]??"rolling magnitude confirms the active state"}</strong></article>})}</div><footer><b>Interpretation:</b> {quartet.explanation}<small>Normalized value = clip((Greek − rolling mean) / rolling standard deviation, −3, 3) / 3. Speed alone supplies direction because volatility-curvature signs require a full strike and IV surface to interpret directionally.</small></footer></section>;
}
...
function DeltaDynamicsEventLog({history,state,symbol,calls=[]}){
  ...
  const copy=async()=>{
    const headers=["Direction","Zone","Date · Eastern","Time · MS","Market Hour","Source","Datum / Alert Price","Session Target","Strike / Reference","Dynamic / Extreme High","High Change","Time to High","Dynamic / Extreme Low","Low Change","Time to Low","Current / Final","Call State","Zone Match","Confidence",...SIX_GREEKS.flatMap(name=>[`${name.toUpperCase()} High`,`${name.toUpperCase()} Low`]),"Event ID"];
    const parentRows=visible.map(row=>{const call=row.call,high=number(call.highest_price,call.dynamic_high),low=number(call.lowest_price,call.dynamic_low),current=number(call.current_price??call.final_price??call.minute_bars?.at(-1)?.close,NaN),change=value=>Number.isFinite(value)?`${value-row.datum>=0?"+":""}${(value-row.datum).toFixed(4)} pts`:"—";return [row.decision==="UP"?"UPWARD":"DOWNWARD",pretty(row.zone),logDate(row.timestamp),logTime(row.timestamp),marketHourLabel(row.timestamp),row.symbol,row.datum.toFixed(4),row.reach.toFixed(4),`${row.strike.estimated?"REF ":""}${row.strike.value.toFixed(2)}`,Number.isFinite(high)?high.toFixed(4):"—",change(high),duration(call.seconds_to_high),Number.isFinite(low)?low.toFixed(4):"—",change(low),duration(call.seconds_to_low),Number.isFinite(current)?current.toFixed(4):"—",call.status??"TRACKING",pct(row.score),pct(row.confidence),...SIX_GREEKS.flatMap(name=>{const signal=call.greek_values_at_signal?.[name];return [signedGreek(call.greek_values_highest?.[name]??signal),signedGreek(call.greek_values_lowest?.[name]??signal)]}),row.id]});
...
function GammaDynamicsChart({history=[],state,symbol,deltaMode=false}){
  ...
  const gammaModelFor=row=>deriveGammaDynamics(row?{...row,gamma_dynamics:null}:row,rows.filter(candidate=>new Date(candidate.timestamp)<new Date(row?.timestamp)).slice(-100)),valueFor=(row,name)=>deltaMode?optionalNumber(row?.zone_intelligence?.normalized?.[name]??greekValue(row,name)):optionalNumber(row?.gamma_dynamics?.normalized?.[name]??gammaModelFor(row).normalized?.[name]),formatValue=value=>Number.isFinite(Number(value))?`${number(value)>=0?"+":""}${number(value).toFixed(3)}`:"--",formatTime=chartTime;
  ...
  return <ChartShell expanded={expanded} setExpanded={setExpanded} className={`gamma-dynamics-history-chart gamma-dynamics-chart ${deltaMode?"delta-dynamics-chart":""}`}>
    <div className="greek-chart-header"><div><span>{deltaMode?"DELTA DYNAMICS HISTORY":"GAMMA DYNAMICS HISTORY · NORMALIZED LIVE STREAM"}</span><h2>{symbol} · {deltaMode?"Ultima / Zomma / Gamma / Speed / Color / Delta":"Zomma / Color / Speed / Gamma / Ultima / Vomma"}</h2></div><div className="greek-header-actions"><ChartTimeControls {...{intervalSeconds,setIntervalSeconds,isLive:viewport.isLive,setOffset:viewport.setOffset,expanded,setExpanded,zoom,onZoom:zoomChart}}/></div></div>
...
function GammaDynamicsLog({history,state,symbol,calls=[]}){
  ...
  const events=useMemo(()=>{
    ...
    const persisted=calls.map(call=>{
      ...
      return {
        ...
        normalized:snapshot.normalized??{},...Object.fromEntries(GAMMA_DYNAMICS_GREEKS.map(name=>[name,Number(inputs[name])])),
        persisted:true,
      };
    });
  ...
  const copySummaries=async()=>{
    const headers=["Direction","Call State","Date · Eastern","Time · MS","Market Hour","Source","Datum / Alert Price","Session Target","Strike / Reference","Dynamic / Extreme High","High Change","Time to High","Dynamic / Extreme Low","Low Change","Time to Low","Current / Final","Current / Final Change","Strongest Greek","Weakest Greek","Intensity","Pressure",...GAMMA_DYNAMICS_GREEKS.flatMap(name=>[`${name.toUpperCase()} @ Strike · Raw`,`${name.toUpperCase()} @ Strike · Normalized`,`${name.toUpperCase()} High`,`${name.toUpperCase()} Low`]),...(["Strike","Success","Failure"].flatMap(phase=>GAMMA_RANKS.map(rank=>`${phase} · ${pretty(rank)}`))),"Event ID"];
    const parentRows=visibleEvents.map(event=>{const call=linkedCall(event),view=visualState(call),finished=Boolean(call&&callOutcome(call).closed),high=call?number(call.highest_price,call.dynamic_high):NaN,low=call?number(call.lowest_price,call.dynamic_low):NaN,datum=number(call?.entry_price,event.price),currentPrice=number(call?.current_price??call?.final_price??call?.minute_bars?.at(-1)?.close,NaN),strike=referenceStrike(call??event,datum),signalRanks=callRankings(call,"signal"),successRanks=callRankings(call,"target"),failureRanks=callRankings(call,"failure"),change=value=>Number.isFinite(value)?`${value-datum>=0?"+":""}${(value-datum).toFixed(4)} pts`:"—";return [event.decision==="UP"?"UPWARD":"DOWNWARD",view.label,logDate(event.timestamp),logTime(event.timestamp),marketHourLabel(event.timestamp),event.symbol,datum.toFixed(4),number(call?.target_price,gammaReachFromDatum(datum,event.decision,event.timestamp)).toFixed(4),Number.isFinite(strike.value)?`${strike.estimated?"REF ":""}${strike.value.toFixed(2)}`:"—",Number.isFinite(high)?high.toFixed(4):"—",change(high),finished?duration(call?.seconds_to_high):"TRACKING",Number.isFinite(low)?low.toFixed(4):"—",change(low),finished?duration(call?.seconds_to_low):"TRACKING",Number.isFinite(currentPrice)?currentPrice.toFixed(4):"—",change(currentPrice),call?.strongest_greek_current??call?.strongest_greek??"—",call?.weakest_greek_current??call?.weakest_greek??"—",Number.isFinite(event.intensity)?pct(event.intensity):"—",Number.isFinite(event.pressure)?`${event.pressure>0?"+":""}${event.pressure.toFixed(2)}`:"—",...GAMMA_DYNAMICS_GREEKS.flatMap(name=>{const strikeValue=call?.greek_values_at_signal?.[name]??event[name];return [signedGreek(strikeValue),Number.isFinite(Number(call?.gamma_dynamics_at_signal?.normalized?.[name]??event.normalized?.[name]))?number(call?.gamma_dynamics_at_signal?.normalized?.[name]??event.normalized?.[name]).toFixed(3):"—",signedGreek(call?.greek_values_highest?.[name]??strikeValue),signedGreek(call?.greek_values_lowest?.[name]??strikeValue)]}),...GAMMA_RANKS.map(rank=>rankingText(signalRanks,rank)),...GAMMA_RANKS.map(rank=>rankingText(successRanks,rank)),...GAMMA_RANKS.map(rank=>rankingText(failureRanks,rank)),event.id]});
    ...
    try{await navigator.clipboard.writeText([exportTsv(`GAMMA DYNAMICS EVENT LOG · ${symbol} · EASTERN TIME`,headers,parentRows),exportTsv("GAMMA DYNAMICS CHILD LEGS",childHeaders,childRows)].join("\n\n"));setCopied(true);window.setTimeout(()=>setCopied(false),1800)}catch{setCopied(false)}
  };
  const gateItems=[
  ...
  return <article className="panel gamma-dynamics-log">
    ...
    <div className="gamma-log-scroll-shell">
      ...
      <thead><tr><th>DIRECTION</th><th>CALL STATE</th><th>DATE · EASTERN</th><th>TIME · MS</th><th>MARKET HOUR</th><th>SOURCE</th><th>DATUM / ALERT PRICE</th><th>SESSION TARGET</th><th>STRIKE / REFERENCE</th><th>DYNAMIC / EXTREME HIGH</th><th>HIGH CHANGE</th><th>TIME TO HIGH</th><th>DYNAMIC / EXTREME LOW</th><th>LOW CHANGE</th><th>TIME TO LOW</th><th>CURRENT / FINAL</th><th>CURRENT / FINAL CHANGE</th><th>STRONGEST GREEK</th><th>WEAKEST GREEK</th><th>INTENSITY</th><th>PRESSURE</th>{GAMMA_DYNAMICS_GREEKS.flatMap(name=>[<th key={`${name}-strike`}>{name.toUpperCase()} @ STRIKE · RAW</th>,<th key={`${name}-normalized`}>{name.toUpperCase()} @ STRIKE · NORM</th>,<th key={`${name}-high`}>{name.toUpperCase()} HIGH</th>,<th key={`${name}-low`}>{name.toUpperCase()} LOW</th>])}{["STRIKE","SUCCESS","FAILURE"].flatMap(phase=>GAMMA_RANKS.map(rank=><th className={`greek-rank-head ${rank}`} key={`${phase}-${rank}`}>{phase} · {rank.toUpperCase()}</th>))}<th>EVENT ID</th></tr></thead>
      <tbody>{visibleEvents.map((event,index)=>{
        ...
        const high=call?number(call.highest_price,expired?snapshot.high:call.dynamic_high):NaN,low=call?number(call.lowest_price,expired?snapshot.low:call.dynamic_low):NaN,datum=number(call?.entry_price,event.price),strike=referenceStrike(call??event,datum),signalRanks=callRankings(call,"signal"),successRanks=callRankings(call,"target"),failureRanks=callRankings(call,"failure");
        return <Fragment key={`${event.timestamp}-${event.decision}-${index}`}>
          <tr className={`gamma-call-${view.key}`}><td><span className={`direction-pill ${event.decision.toLowerCase()}`}>{event.decision==="UP"?"UPWARD":"DOWNWARD"}</span></td><td><span className={`gamma-call-state ${view.key}`}>{view.label}</span></td><td>{logDate(event.timestamp)}</td><td>{logTime(event.timestamp)}</td><td><span className="market-hour">{marketHourLabel(event.timestamp)}</span></td><td>{event.symbol}</td><td className="extreme-price">{Number.isFinite(datum)?datum.toFixed(4):"—"}</td><td className="extreme-price">{Number.isFinite(datum)?number(call?.target_price,gammaReachFromDatum(datum,event.decision,event.timestamp)).toFixed(4):"—"}</td><td className="extreme-price">{Number.isFinite(strike.value)?`${strike.estimated?"REF ":""}${strike.value.toFixed(2)}`:"—"}</td><td className="extreme-price">{priceValue(call,high)}</td><td>{pointValue(call,high)}</td><td className="extreme-time">{extremeTime(call,call?.seconds_to_high,finished)}</td><td className="extreme-price">{priceValue(call,low)}</td><td>{pointValue(call,low)}</td><td className="extreme-time">{extremeTime(call,call?.seconds_to_low,finished)}</td><td className="extreme-price">{priceValue(call,livePrice)}</td><td>{pointValue(call,livePrice)}</td><td><GreekAuditBadge label={call?.strongest_greek_current??call?.strongest_greek}/></td><td><GreekAuditBadge label={call?.weakest_greek_current??call?.weakest_greek}/></td><td>{Number.isFinite(event.intensity)?pct(event.intensity):"—"}</td><td>{Number.isFinite(event.pressure)?`${event.pressure>0?"+":""}${event.pressure.toFixed(2)}`:"—"}</td>{GAMMA_DYNAMICS_GREEKS.flatMap(name=>{const strikeValue=call?.greek_values_at_signal?.[name]??event[name],normalized=call?.gamma_dynamics_at_signal?.normalized?.[name]??event.normalized?.[name];return [<td key={`${name}-strike`}>{signedGreek(strikeValue)}</td>,<td className="greek-normalized" key={`${name}-normalized`}>{Number.isFinite(Number(normalized))?`${number(normalized)>=0?"+":""}${number(normalized).toFixed(3)}`:"—"}</td>,<td className="greek-extreme-high" key={`${name}-high`}>{signedGreek(call?.greek_values_highest?.[name]??strikeValue)}</td>,<td className="greek-extreme-low" key={`${name}-low`}>{signedGreek(call?.greek_values_lowest?.[name]??strikeValue)}</td>]})}{GAMMA_RANKS.map(rank=><td className={`greek-rank-cell ${rank}`} key={`signal-${rank}`}>{rankingText(signalRanks,rank)}</td>)}{GAMMA_RANKS.map(rank=><td className={`greek-rank-cell ${rank}`} key={`target-${rank}`}>{rankingText(successRanks,rank)}</td>)}{GAMMA_RANKS.map(rank=><td className={`greek-rank-cell ${rank}`} key={`failure-${rank}`}>{rankingText(failureRanks,rank)}</td>)}<td><button type="button" className="call-id" onClick={()=>navigator.clipboard.writeText(event.id)} title="Copy event ID">{event.id}</button></td></tr>
...
function FiftyPointPathChart({call}){
  ...
  const childDatums=(call.family_legs??[]).filter(leg=>leg.role==="CHILD").map((leg,index)=>({label:`CHILD ${index+1}`,value:number(leg.datum)})).filter(level=>Number.isFinite(level.value));
...
function FiftyPointOutcomeCard({call,system="PRIMARY_OPTIONS"}){
  ...
  const gammaRankPhase=reached?"target":expired?"failure":"current",gammaRankScores=call[`greek_scores_at_${gammaRankPhase}`]??(gammaRankPhase==="failure"?call.greek_scores_current:{}),gammaRankings=system==="GAMMA_DYNAMICS"?(call[`greek_rankings_at_${gammaRankPhase}`]??greekRankings(gammaRankScores)):null;
...
export default function Home() {
  ...
  return <main className={`workspace focus-${focusTone}`}><header className="topbar"><div className="brand"><div className="brandmark"><span/><span/><span/></div><div><b>AXIOM</b><small>PRESSURE INTELLIGENCE</small></div></div><nav className="mode-switch" aria-label="Engine mode"><button className={view!=="Historical Replay"?"active":""} onClick={()=>setView("Overview")}>Live engine</button><button className={view==="Historical Replay"?"active":""} onClick={()=>setView("Historical Replay")}>Training replay</button></nav><label className="section-jump"><span>Section</span><select value={activeSection} onChange={event=>jumpTo(event.target.value)}>{orderedOverviewSections.map(([label,id])=><option value={id} key={id}>{OVERVIEW_NUMBERS[id]} · {label}</option>)}</select></label><div className="header-actions status-cluster" aria-live="polite"><span className={`status-chip ${connected?"is-good":"is-bad"}`}>API <b>{connected?"ONLINE":"OFFLINE"}</b></span><span className={`status-chip ${engine.running?"is-good":"is-idle"}`}>ENGINE <b>{engine.running?"ON":"IDLE"}</b></span><span className={`status-chip ${dataFresh?"is-good":dataDelayed?"is-bad":"is-idle"}`}>DATA <b>{dataFresh?`${stateAge}s`:dataDelayed?"STALE":"IDLE"}</b></span></div></header>
    ...
    <DraggableOverviewModule id="gamma-dynamics" index={moduleOrder.indexOf("gamma-dynamics")} {...draggableProps}><OverviewDisclosure id="gamma-dynamics" title="Gamma Dynamics · Six-Greek Engine" description="Normalized Zomma/Color/Ultima/Vomma intensity · Speed/Gamma curvature pressure"><GammaDynamicsModule state={state} history={visualHistory} symbol={symbol} engine={engine}/><article className="panel chart-panel triad-history-panel"><GammaDynamicsChart history={visualHistory} state={state} symbol={symbol}/></article><GammaDynamicsLog history={visualHistory} state={state} symbol={symbol} calls={attribution?.systems?.GAMMA_DYNAMICS?.calls??[]}/><OutcomeAttributionMini system="GAMMA_DYNAMICS" data={attribution} symbol={symbol}/></OverviewDisclosure></DraggableOverviewModule>
