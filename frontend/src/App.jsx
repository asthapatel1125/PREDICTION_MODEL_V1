...
const OVERVIEW_SECTIONS = [
  ...
  ["Custom Greek graphs", "custom-greeks"], ["Live alerts", "live-alerts"], ["Triad alert log", "triad-alert-log"],
];
...
function deriveTriadAlerts(history,state,symbol){
  const rows=[...history];
  if(state?.timestamp&&!rows.some(row=>row.timestamp===state.timestamp))rows.push(state);
  rows.sort((a,b)=>new Date(a.timestamp)-new Date(b.timestamp));
  const events=[];
  let prior="NEUTRAL";
  rows.forEach(row=>{const triad=deriveMomentumTriad(row),decision=triad.available&&triad.aligned?triad.decision:"NEUTRAL";if(decision!=="NEUTRAL"&&decision!==prior)events.push({timestamp:row.timestamp,symbol:row.symbol??symbol,decision,zomma:triad.acceleration??triad.zomma,speed:triad.direction??triad.speed,delta:triad.confirmation??triad.delta});prior=decision});
...
function GreekOrderChart({ history = [], state, symbol }) {
  ...
  const firstLaneHeight=plotHeight/Math.max(config.series.length,1);
  const projectValue=value=>scale.project(value)/scale.projectedMax;
  const y=(value,name)=>{if(order!=="first")return dims.top+(1-projectValue(value))*plotHeight/2;const laneIndex=config.series.findIndex(([seriesName])=>seriesName===name),range=firstOrderRanges[name],laneTop=dims.top+laneIndex*firstLaneHeight+9,laneBottom=dims.top+(laneIndex+1)*firstLaneHeight-9;return laneTop+(range.maximum-value)*(laneBottom-laneTop)/Math.max(range.maximum-range.minimum,1e-15)};
  ...
  const onPointerMove=event=>{const next=chartCursor(event,dims,rows.length,1);setCursor({...next,value:order==="first"?next.value:scale.unproject(next.value*scale.projectedMax)});if(drag.current){const delta=event.clientX-drag.current.x;if(Math.abs(delta)>3)drag.current.moved=true;viewport.setOffset(Math.max(0,Math.min(viewport.maxOffset,drag.current.offset-Math.round(delta/7))))}};
  ...
  return <ChartShell expanded={expanded} setExpanded={setExpanded} className={`greek-order-chart order-${order}`}>
    ...
    <div className="greek-chart-stage" onWheel={event=>{event.preventDefault();const anchor=chartCursor(event,dims,rows.length,1);setCursor({...anchor,value:order==="first"?anchor.value:scale.unproject(anchor.value*scale.projectedMax)});if(event.shiftKey)viewport.move(event.deltaY>0?10:-10);else zoomChart(event.deltaY<0?.5:-.5,anchor.index)}} onPointerDown={onPointerDown} onPointerMove={onPointerMove} onPointerUp={onPointerUp} onPointerCancel={()=>{drag.current=null}} onPointerLeave={()=>{drag.current=null;setCursor(null)}}>
      <svg viewBox={`0 0 ${dims.width} ${dims.height}`} role="img" aria-label={`${config.label} live options Greeks over time with signed exposure on the y axis`}>
        <title>{config.label} live options Greeks for {symbol}</title><desc>The horizontal axis is observation time. First-order Greeks use separate real-value lanes; other orders share a signed scale.</desc>
        {order==="first"?config.series.flatMap(([name,color],laneIndex)=>{const range=firstOrderRanges[name],laneTop=dims.top+laneIndex*firstLaneHeight,laneBottom=dims.top+(laneIndex+1)*firstLaneHeight,laneMiddle=(laneTop+laneBottom)/2;return <g key={`lane-${name}`}><rect className="first-order-lane" x={dims.left} y={laneTop} width={plotWidth} height={firstLaneHeight}/><text className="first-order-lane-name" x="14" y={laneMiddle+5} style={{fill:color}}>{name.toUpperCase()}</text>{[range.maximum,(range.maximum+range.minimum)/2,range.minimum].map((tickValue,tickIndex)=>{const yy=[laneTop+9,laneMiddle,laneBottom-9][tickIndex];return <g key={`${name}-${tickIndex}`}><line className="greek-grid" x1={dims.left} x2={dims.width-dims.right} y1={yy} y2={yy}/><text className="greek-axis" x={dims.left-10} y={yy+4} textAnchor="end">{formatValue(tickValue)}</text></g>})}</g>}):[0,1,2,3,4].map(tick=>{const projected=1-tick/2,value=scale.unproject(projected*scale.projectedMax),yy=yProjected(projected);return <g key={`gy-${tick}`}><line className="greek-grid" x1={dims.left} x2={dims.width-dims.right} y1={yy} y2={yy}/><text className="greek-axis" x={dims.left-10} y={yy+4} textAnchor="end">{formatValue(value)}</text></g>})}
        ...
        {order!=="first"&&<line className="greek-zero" x1={dims.left} x2={dims.width-dims.right} y1={yProjected(0)} y2={yProjected(0)}/>}
        ...
        {cursor&&<g><line className="greek-crosshair" x1={x(cursor.index)} x2={x(cursor.index)} y1={dims.top} y2={dims.height-dims.bottom}/>{order!=="first"&&<line className="greek-crosshair horizontal" x1={dims.left} x2={dims.width-dims.right} y1={y(cursor.value)} y2={y(cursor.value)}/>} {config.series.map(([name,color])=>{const value=seriesValue(hovered,name);return Number.isFinite(value)?<circle key={name} className="greek-hover-dot" cx={x(cursor.index)} cy={y(value,name)} r="4" style={{fill:color}}/>:null})}</g>} 
      </svg>
      <ChartCoordinateTooltip {...{cursor,row:hovered,series:config.series,formatTime,formatValue,seriesValue}} cursorValueText={order==="first"?()=>"Actual values by lane":undefined}/>
...
function CustomGreekChart({chart,index,history,state,symbol,onChange,onRemove,canRemove}){
  ...
  return <ChartShell expanded={expanded} setExpanded={setExpanded} className="custom-greek-chart">
    <div className="greek-chart-header"><div><span>CUSTOM GREEK GRAPH {String(index+1).padStart(2,"0")}</span><h2>{symbol} · selected live exposures</h2></div><div className="custom-chart-actions"><ChartTimeControls {...{intervalSeconds,setIntervalSeconds,isLive:viewport.isLive,setOffset:viewport.setOffset,expanded,setExpanded,zoom,onZoom:zoomChart}}/>{canRemove&&<button className="remove-chart" onClick={onRemove}>Remove</button>}</div></div>
    <GreekMultiSelect selected={chart.selected} onChange={selected=>onChange({...chart,selected})}/>
    ...
    <div className="greek-chart-stage" onWheel={event=>{event.preventDefault();if(event.ctrlKey||event.metaKey)zoomChart(event.deltaY<0?.5:-.5);else viewport.move(event.deltaY>0?10:-10)}} onPointerDown={onPointerDown} onPointerMove={onPointerMove} onPointerUp={onPointerUp} onPointerLeave={()=>{drag.current=null;setCursor(null)}}>
      <svg viewBox={`0 0 ${dims.width} ${dims.height}`} role="img" aria-label={`Custom live Greek chart ${index+1} showing ${chart.selected.join(", ")}`}><title>Custom Greek graph {index+1}</title><desc>Selected open-interest-weighted Greek exposures over streamed time.</desc>
        ...
        {[0,1,2,3,4,5].map(tick=>{const point=Math.round(tick*Math.max(rows.length-1,0)/5),xx=x(point);return <g key={`gx-${tick}`}><line className="greek-grid" x1={xx} x2={xx} y1={dims.top} y2={dims.height-dims.bottom}/><text className="greek-axis" x={xx} y={dims.height-15} textAnchor="middle">{formatTime(rows[point]?.timestamp)}</text></g>})}
        <line className="greek-zero" x1={dims.left} x2={dims.width-dims.right} y1={y(0)} y2={y(0)}/>
        {series.map(([name,color])=>{let drawing=false;const path=rows.map((row,point)=>{const value=seriesValue(row,name);if(!Number.isFinite(value)){drawing=false;return ""}const command=drawing?"L":"M";drawing=true;return `${command}${x(point).toFixed(1)},${y(value).toFixed(1)}`}).filter(Boolean).join(" "),latest=seriesValue(rows.at(-1),name);return <g key={name}>{path&&<path className="greek-series" d={path} style={{stroke:color}}/>}{Number.isFinite(latest)&&<circle className="greek-current-dot" cx={x(rows.length-1)} cy={y(latest)} r="3.5" style={{fill:color}}/>}</g>})}
        {cursor&&<g><line className="greek-crosshair" x1={x(cursor.index)} x2={x(cursor.index)} y1={dims.top} y2={dims.height-dims.bottom}/><line className="greek-crosshair horizontal" x1={dims.left} x2={dims.width-dims.right} y1={y(cursor.value)} y2={y(cursor.value)}/>{series.map(([name,color])=>{const value=seriesValue(hovered,name);return Number.isFinite(value)?<circle key={name} className="greek-hover-dot" cx={x(cursor.index)} cy={y(value)} r="4" style={{fill:color}}/>:null})}</g>}
      </svg>
      <ChartCoordinateTooltip {...{cursor,row:hovered,series,formatTime,formatValue,seriesValue}}/>{!rows.length&&<div className="chart-empty">Waiting for live Options Pro states…</div>}
    </div>
...
function MomentumTriadChart({history=[],state,symbol}){
  const series=[["zomma","#06d6a0","ACCELERATION"],["speed","#ef476f","DIRECTION"],["delta","#ff5c8a","CONFIRMATION"]];
  const [intervalSeconds,setIntervalSeconds]=useState(5),[expanded,setExpanded]=useState(false),[zoom,setZoom]=useState(1),[cursor,setCursor]=useState(null);
  ...
  return <ChartShell expanded={expanded} setExpanded={setExpanded} className="momentum-triad-chart">
    <div className="greek-chart-header"><div><span>NQ MOMENTUM TRIAD HISTORY</span><h2>{symbol} options proxy · value vs time</h2></div><div className="greek-header-actions"><ChartTimeControls {...{intervalSeconds,setIntervalSeconds,isLive:viewport.isLive,setOffset:viewport.setOffset,expanded,setExpanded,zoom,onZoom:zoomChart}}/></div></div>
    ...
    <div className="greek-chart-stage" onWheel={event=>{event.preventDefault();const anchor=cursorAt(event);setCursor(anchor);if(event.shiftKey)viewport.move(event.deltaY>0?10:-10);else zoomChart(event.deltaY<0?.5:-.5,anchor.index)}} onPointerDown={onPointerDown} onPointerMove={onPointerMove} onPointerUp={onPointerUp} onPointerCancel={()=>{drag.current=null}} onPointerLeave={()=>{drag.current=null;setCursor(null)}}>
      <svg viewBox={`0 0 ${dims.width} ${dims.height}`} role="img" aria-label="Zomma acceleration, Speed direction, and Delta confirmation plotted over time">
      ...
      </svg><ChartCoordinateTooltip {...{cursor,row:hovered,series:series.map(([name,color])=>[name,color]),formatTime,formatValue,seriesValue:valueFor}} cursorValueText={()=>"Actual values by lane"}/>{!rows.length&&<div className="chart-empty">Waiting for streamed triad history...</div>}
    </div>
...
export default function Home() {
  ...
  useEffect(()=>{if(view!=="Overview")return;const sections=OVERVIEW_SECTIONS.map(([,id])=>document.getElementById(id)).filter(Boolean);const observer=new IntersectionObserver(entries=>{const visible=entries.filter(entry=>entry.isIntersecting).sort((a,b)=>b.intersectionRatio-a.intersectionRatio)[0];if(visible)setActiveSection(visible.target.id)},{rootMargin:"-18% 0px -68% 0px",threshold:[0,.2,.5,.8]});sections.forEach(section=>observer.observe(section));return()=>observer.disconnect()},[view]);
  ...
  const triadAlerts=useMemo(()=>deriveTriadAlerts(visualHistory,state,symbol),[visualHistory,state,symbol]);
  ...
  return <main className={`workspace focus-${focusTone}`}><header className="topbar"><div className="brand"><div className="brandmark"><span/><span/><span/></div><div><b>AXIOM</b><small>PRESSURE INTELLIGENCE</small></div></div><nav className="mode-switch"><button className={view!=="Historical Replay"?"active":""} onClick={()=>setView("Overview")}>Live engine</button><button className={view==="Historical Replay"?"active":""} onClick={()=>setView("Historical Replay")}>Training replay</button></nav><label className="section-jump"><span>Jump to</span><select value={activeSection} onChange={event=>jumpTo(event.target.value)}>{OVERVIEW_SECTIONS.map(([label,id])=><option value={id} key={id}>{label}</option>)}</select></label><div className="header-actions status-cluster" aria-live="polite"><span className={`status-chip ${connected?"is-good":"is-bad"}`}><i/>API <b>{connected?"ONLINE":"OFFLINE"}</b></span><span className={`status-chip ${engine.running?"is-good":"is-idle"}`}><i/>ENGINE <b>{engine.running?"ON":"IDLE"}</b></span><span className={`status-chip ${dataFresh?"is-good":dataDelayed?"is-bad":"is-idle"}`}><i/>DATA <b>{dataFresh?`${stateAge}s`:dataDelayed?"STALE":"IDLE"}</b></span></div></header>
    ...
    <article className="panel chart-panel triad-history-panel"><MomentumTriadChart history={visualHistory} state={state} symbol={symbol}/></article>
    ...
    <section id="triad-alert-log" className="overview-section triad-alert-log"><OverviewSectionHeading number="06" title="NQ momentum triad alert log" description="A new row is recorded only when all three signs align into a new LONG or SHORT state."/><article className="panel alerts-panel"><header className="panel-head table-head"><div><span>MOMENTUM TRIAD ALIGNMENT EVENTS</span><h2>Zomma acceleration · Speed direction · Delta confirmation</h2></div><span className="triad-log-count">{triadAlerts.length} EVENTS</span></header><div className="table-wrap"><table><thead><tr><th>TIME</th><th>SOURCE</th><th>NQ PROXY</th><th>ZOMMA</th><th>SPEED</th><th>DELTA</th><th>ALIGNMENT</th></tr></thead><tbody>{triadAlerts.map((alert,index)=><tr key={`${alert.timestamp}-${alert.decision}-${index}`}><td>{new Date(alert.timestamp).toLocaleString()}</td><td><b>{alert.symbol}</b></td><td><span className={`direction-pill ${alert.decision.toLowerCase()}`}>{biasLabel(alert.decision)}</span></td><td>{signedGreek(alert.zomma)}</td><td>{signedGreek(alert.speed)}</td><td>{signedGreek(alert.delta)}</td><td>3 / 3</td></tr>)}</tbody></table>{!triadAlerts.length&&<div className="empty-state">No triad alignment event is present in the loaded streamed history.</div>}</div></article></section></>}
