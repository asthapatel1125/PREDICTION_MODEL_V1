...
function GammaDynamicsLog({history,state,symbol,calls=[]}){
  ...
  const familyTone=call=>number(call?.family_average_pl_points)>0?"profit":number(call?.family_average_pl_points)<0?"loss":"flat";
  const familyTime=call=>{
    if(!call?.expires_at)return "—";
    if(callOutcome(call).closed)return "COMPLETE";
    return duration(Math.max(0,(new Date(call.expires_at).getTime()-Date.now())/1000));
  };
  const copySummaries=async()=>{
    const headers=["Call ID","Status","Direction","Alerted (ET)","Datum","Family Stage","Active Legs","Average Datum","Family Avg P/L","Next Child","Family Time Left","Extreme High","High Change","Time to High","Extreme Low","Low Change","Time to Low","Current / Final","Final Change","Family Legs"];
    const clean=value=>String(value??"—").replaceAll("|","/");
    const rows=visibleEvents.map(event=>{const call=linkedCall(event),view=visualState(call),finished=Boolean(call&&callOutcome(call).closed),high=call?number(call.highest_price,call.dynamic_high):NaN,low=call?number(call.lowest_price,call.dynamic_low):NaN,datum=number(call?.entry_price,event.price),currentPrice=number(call?.current_price??call?.final_price??call?.minute_bars?.at(-1)?.close,NaN),familyPl=Number(call?.family_average_pl_points),legs=(call?.family_legs??[]).map(leg=>`${leg.call_id}: datum ${number(leg.datum).toFixed(4)}, P/L ${number(leg.current_pl_points)>=0?"+":""}${number(leg.current_pl_points).toFixed(4)} pts`).join("; ");return [event.id,view.label,event.decision,`${logDate(event.timestamp)} ${logTime(event.timestamp)}`,datum.toFixed(4),call?.family_stage??"—",call?.family_active_legs??"—",Number.isFinite(Number(call?.family_average_datum))?number(call.family_average_datum).toFixed(4):"—",Number.isFinite(familyPl)?`${familyPl>=0?"+":""}${familyPl.toFixed(4)} pts`:"—",!call?"—":call.family_next_trigger_points==null?"NONE":`${event.decision==="UP"?"-":"+"}${number(call.family_next_trigger_points).toFixed(0)} pts`,familyTime(call),Number.isFinite(high)?high.toFixed(4):"—",Number.isFinite(high)?`${high-datum>=0?"+":""}${(high-datum).toFixed(4)} pts`:"—",finished?duration(call.seconds_to_high):"Tracking",Number.isFinite(low)?low.toFixed(4):"—",Number.isFinite(low)?`${low-datum>=0?"+":""}${(low-datum).toFixed(4)} pts`:"—",finished?duration(call.seconds_to_low):"Tracking",Number.isFinite(currentPrice)?currentPrice.toFixed(4):"—",Number.isFinite(currentPrice)?`${currentPrice-datum>=0?"+":""}${(currentPrice-datum).toFixed(4)} pts`:"—",legs||"—"].map(clean)});
  ...
  return <article className="panel gamma-dynamics-log">
    ...
    <div className="gamma-log-scroll-shell">
      ...
      <thead><tr><th>DIRECTION</th><th>DATE · EASTERN</th><th>TIME · MS</th><th>SOURCE</th><th>ALERT PRICE</th><th>RISK FAMILY</th><th>LEGS</th><th>AVERAGE DATUM</th><th>FAMILY AVG P/L</th><th>NEXT CHILD</th><th>FAMILY TIME LEFT</th><th>DYNAMIC / EXTREME HIGH</th><th>HIGH CHANGE</th><th>TIME TO HIGH</th><th>DYNAMIC / EXTREME LOW</th><th>LOW CHANGE</th><th>TIME TO LOW</th><th>CURRENT / FINAL</th><th>CURRENT / FINAL CHANGE</th><th>CALL STATE</th><th>INTENSITY</th><th>PRESSURE</th><th>ZOMMA</th><th>COLOR</th><th>SPEED</th><th>GAMMA</th><th>EVENT ID</th></tr></thead>
      <tbody>{visibleEvents.map((event,index)=>{
        ...
        return <Fragment key={`${event.timestamp}-${event.decision}-${index}`}>
          <tr className={`gamma-call-${view.key}`}><td><span className={`direction-pill ${event.decision.toLowerCase()}`}>{event.decision==="UP"?"UPWARD":"DOWNWARD"}</span></td><td>{logDate(event.timestamp)}</td><td>{logTime(event.timestamp)}</td><td>{event.symbol}</td><td className="extreme-price">{Number.isFinite(event.price)?event.price.toFixed(4):"—"}</td><td><span className={`gamma-family-state ${familyTone(call)}`}>{call?.family_stage??"—"}</span></td><td className="family-legs-count">{call?`${call.family_active_legs??1} / 4`:"—"}</td><td className="extreme-price">{priceValue(call,call?.family_average_datum)}</td><td><span className={`family-pl ${familyTone(call)}`}>{Number.isFinite(Number(call?.family_average_pl_points))?`${number(call.family_average_pl_points)>=0?"+":""}${number(call.family_average_pl_points).toFixed(4)} pts`:"—"}</span></td><td className="family-next">{!call?"—":call.family_next_trigger_points==null?"NONE":`${event.decision==="UP"?"-":"+"}${number(call.family_next_trigger_points).toFixed(0)} pts`}</td><td className="extreme-time">{familyTime(call)}</td><td className="extreme-price">{priceValue(call,high)}</td><td>{pointValue(call,high)}</td><td className="extreme-time">{extremeTime(call,call?.seconds_to_high,finished)}</td><td className="extreme-price">{priceValue(call,low)}</td><td>{pointValue(call,low)}</td><td className="extreme-time">{extremeTime(call,call?.seconds_to_low,finished)}</td><td className="extreme-price">{priceValue(call,livePrice)}</td><td>{pointValue(call,livePrice)}</td><td><span className={`gamma-call-state ${view.key}`}>{view.label}</span></td><td>{Number.isFinite(event.intensity)?pct(event.intensity):"—"}</td><td>{Number.isFinite(event.pressure)?`${event.pressure>0?"+":""}${event.pressure.toFixed(2)}`:"—"}</td><td>{signedGreek(event.zomma)}</td><td>{signedGreek(event.color)}</td><td>{signedGreek(event.speed)}</td><td>{signedGreek(event.gamma)}</td><td><button type="button" className="call-id" onClick={()=>navigator.clipboard.writeText(event.id)} title="Copy event ID">{event.id}</button></td></tr>
          {call&&<tr className={`gamma-call-chart-row gamma-call-${view.key}`}><td colSpan="27"><details><summary>RISK FAMILY + GRAPH · {event.id} · 0 / {event.decision==="UP"?"-4 / -6 / -8":"+4 / +6 / +8"}</summary><div className="gamma-family-legs">{(call.family_legs??[]).map(leg=><div className={`gamma-family-leg ${number(leg.current_pl_points)>0?"profit":number(leg.current_pl_points)<0?"loss":"flat"}`} key={leg.call_id}><span>LEG {leg.leg_number} · {leg.role}</span><button type="button" onClick={()=>navigator.clipboard.writeText(leg.call_id)}>{leg.call_id}</button><b>DATUM {number(leg.datum).toFixed(4)}</b><strong>{number(leg.current_pl_points)>=0?"+":""}{number(leg.current_pl_points).toFixed(4)} PTS</strong><small>TRIGGER {leg.trigger_adverse_points===0?"0":`${event.decision==="UP"?"-":"+"}${number(leg.trigger_adverse_points).toFixed(0)}`} · {logTime(leg.activated_at)}</small></div>)}</div><FiftyPointPathChart call={call}/></details></td></tr>}
        </Fragment>
      })}</tbody>
