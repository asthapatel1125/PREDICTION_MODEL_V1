    ...
    <div className="gamma-log-scroll-shell">
      ...
      <tbody>{visibleEvents.map((event,index)=>{
        ...
        return <Fragment key={`${event.timestamp}-${event.decision}-${index}`}>
          <tr className={`gamma-call-${view.key}`}><td><span className={`direction-pill ${event.decision.toLowerCase()}`}>{event.decision==="UP"?"UPWARD":"DOWNWARD"}</span></td><td>{logDate(event.timestamp)}</td><td>{logTime(event.timestamp)}</td><td><span className="market-hour">{marketHourLabel(event.timestamp)}</span></td><td>{event.symbol}</td><td className="extreme-price">{Number.isFinite(event.price)?event.price.toFixed(4):"—"}</td><td className="extreme-price">{priceValue(call,high)}</td><td>{pointValue(call,high)}</td><td className="extreme-time">{extremeTime(call,call?.seconds_to_high,finished)}</td><td className="extreme-price">{priceValue(call,low)}</td><td>{pointValue(call,low)}</td><td className="extreme-time">{extremeTime(call,call?.seconds_to_low,finished)}</td><td className="extreme-price">{priceValue(call,livePrice)}</td><td>{pointValue(call,livePrice)}</td><td><span className={`gamma-call-state ${view.key}`}>{view.label}</span></td><td><GreekAuditBadge label={call?.strongest_greek_current??call?.strongest_greek}/></td><td><GreekAuditBadge label={call?.weakest_greek_current??call?.weakest_greek}/></td><td>{Number.isFinite(event.intensity)?pct(event.intensity):"—"}</td><td>{Number.isFinite(event.pressure)?`${event.pressure>0?"+":""}${event.pressure.toFixed(2)}`:"—"}</td><td>{signedGreek(event.zomma)}</td><td>{signedGreek(event.color)}</td><td>{signedGreek(event.speed)}</td><td>{signedGreek(event.gamma)}</td><td><button type="button" className="call-id" onClick={()=>navigator.clipboard.writeText(event.id)} title="Copy event ID">{event.id}</button></td></tr>
        </Fragment>
      })}</tbody>
