...
function GammaDynamicsLog({history,state,symbol,calls=[]}){
  ...
  const copySummaries=async()=>{
    const headers=["Direction","Call State","Date · Eastern","Time · MS","Market Hour","Source","Datum / Alert Price","Session Target","Strike / Reference","Dynamic / Extreme High","High Change","Time to High","Dynamic / Extreme Low","Low Change","Time to Low","Current / Final","Current / Final Change","Strongest Greek","Weakest Greek","Intensity","Pressure",...GAMMA_DYNAMICS_GREEKS.flatMap(name=>[`${name.toUpperCase()} @ Strike`,`${name.toUpperCase()} High`,`${name.toUpperCase()} Low`]),"Event ID"];
    const parentRows=visibleEvents.map(event=>{const call=linkedCall(event),view=visualState(call),finished=Boolean(call&&callOutcome(call).closed),high=call?number(call.highest_price,call.dynamic_high):NaN,low=call?number(call.lowest_price,call.dynamic_low):NaN,datum=number(call?.entry_price,event.price),currentPrice=number(call?.current_price??call?.final_price??call?.minute_bars?.at(-1)?.close,NaN),strike=referenceStrike(call??event,datum),change=value=>Number.isFinite(value)?`${value-datum>=0?"+":""}${(value-datum).toFixed(4)} pts`:"—";return [event.decision==="UP"?"UPWARD":"DOWNWARD",view.label,logDate(event.timestamp),logTime(event.timestamp),marketHourLabel(event.timestamp),event.symbol,datum.toFixed(4),number(call?.target_price,reachFromDatum(datum,event.decision,event.timestamp)).toFixed(4),Number.isFinite(strike.value)?`${strike.estimated?"REF ":""}${strike.value.toFixed(2)}`:"—",Number.isFinite(high)?high.toFixed(4):"—",change(high),finished?duration(call?.seconds_to_high):"TRACKING",Number.isFinite(low)?low.toFixed(4):"—",change(low),finished?duration(call?.seconds_to_low):"TRACKING",Number.isFinite(currentPrice)?currentPrice.toFixed(4):"—",change(currentPrice),call?.strongest_greek_current??call?.strongest_greek??"—",call?.weakest_greek_current??call?.weakest_greek??"—",Number.isFinite(event.intensity)?pct(event.intensity):"—",Number.isFinite(event.pressure)?`${event.pressure>0?"+":""}${event.pressure.toFixed(2)}`:"—",...GAMMA_DYNAMICS_GREEKS.flatMap(name=>{const strikeValue=call?.greek_values_at_signal?.[name]??event[name];return [signedGreek(strikeValue),signedGreek(call?.greek_values_highest?.[name]??strikeValue),signedGreek(call?.greek_values_lowest?.[name]??strikeValue)]}),event.id]});
