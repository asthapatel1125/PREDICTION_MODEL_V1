...
const OVERVIEW_SECTIONS = [
  ["One-screen focus", "decision"], ["NQ momentum triad", "momentum-triad"], ["Gamma dynamics", "gamma-dynamics"], ["Signal scores", "score-modules"], ["Greek orders", "greek-orders"],
...
function GammaDynamicsModule({state,history,symbol,engine}){
  ...
  return <section className={`gamma-dynamics gamma-dynamics-${tone}`} aria-live="polite"><header><div><span>GAMMA DYNAMICS QUARTET</span><h2>{symbol} · {label}</h2><small>{engine.running?"● LIVE OPTIONS PRO":"○ ENGINE IDLE"} · relative to the latest {quartet.history_points??0} observations</small></div><div className="gamma-dynamics-score"><span>DYNAMICS INTENSITY</span><b>{pct(quartet.intensity)}</b><small>Threshold {pct(quartet.intensity_threshold??.65)}</small></div><div className="gamma-dynamics-score pressure"><span>CURVATURE PRESSURE</span><b>{number(quartet.pressure)>0?"+":""}{number(quartet.pressure).toFixed(2)}</b><small>{quartet.qualified?"QUALIFIED":"NOT QUALIFIED"}</small></div></header><div className="gamma-dynamics-grid">{["zomma","color","speed","gamma"].map(name=>{const value=quartet.inputs?.[name],rank=quartet.percentiles?.[name]??0;return <article key={name}><div><span>{name.toUpperCase()}</span><small>{metadata[name][0]}</small></div><b>{signedGreek(value)}</b><i><em style={{width:`${Math.max(2,rank*100)}%`}}/></i><p>{metadata[name][1]} · relative magnitude {pct(rank)}</p></article>})}</div><footer><b>Interpretation:</b> {quartet.explanation}<small>Zomma and Color measure intensity/instability; Gamma and Speed determine signed curvature pressure. This is a relative options-risk state, not a standalone price forecast.</small></footer></section>;
}
...
export default function Home() {
  ...
  return <main className={`workspace focus-${focusTone}`}><header className="topbar"><div className="brand"><div className="brandmark"><span/><span/><span/></div><div><b>AXIOM</b><small>PRESSURE INTELLIGENCE</small></div></div><nav className="mode-switch"><button className={view!=="Historical Replay"?"active":""} onClick={()=>setView("Overview")}>Live engine</button><button className={view==="Historical Replay"?"active":""} onClick={()=>setView("Historical Replay")}>Training replay</button></nav><label className="section-jump"><span>Jump to</span><select value={activeSection} onChange={event=>jumpTo(event.target.value)}>{OVERVIEW_SECTIONS.map(([label,id])=><option value={id} key={id}>{label}</option>)}</select></label><div className="header-actions status-cluster" aria-live="polite"><span className={`status-chip ${connected?"is-good":"is-bad"}`}><i/>API <b>{connected?"ONLINE":"OFFLINE"}</b></span><span className={`status-chip ${engine.running?"is-good":"is-idle"}`}><i/>ENGINE <b>{engine.running?"ON":"IDLE"}</b></span><span className={`status-chip ${dataFresh?"is-good":dataDelayed?"is-bad":"is-idle"}`}><i/>DATA <b>{dataFresh?`${stateAge}s`:dataDelayed?"STALE":"IDLE"}</b></span></div></header>
    ...
    <OverviewDisclosure id="momentum-triad" title="NQ Momentum Triad" description="Zomma acceleration · Speed direction · Delta confirmation"><NQMomentumTriadModule state={state} symbol={symbol} engine={engine}/><article className="panel chart-panel triad-history-panel"><MomentumTriadChart history={visualHistory} state={state} symbol={symbol}/></article></OverviewDisclosure>
    <OverviewDisclosure id="gamma-dynamics" title="Gamma Dynamics Quartet" description="Zomma/Color intensity · Speed/Gamma signed curvature pressure"><GammaDynamicsModule state={state} history={visualHistory} symbol={symbol} engine={engine}/><article className="panel chart-panel triad-history-panel"><MomentumTriadChart history={visualHistory} state={state} symbol={symbol} variant="quartet"/></article><GammaDynamicsLog history={visualHistory} state={state} symbol={symbol}/></OverviewDisclosure>
