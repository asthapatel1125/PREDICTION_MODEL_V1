...
import {
  fetchChart, fetchConfiguration, fetchDashboard, fetchDynamicsHistory, fetchInstruments, fetchOutcomeAttribution, fetchOutcomeCall, fetchReplay, fetchSystem, fetchWallSpectrum, fetchWallBreaks, fetchWallDealerFlow, fetchWallSummaryHistory,
...
function WallStrengthDashboard({symbol}){
  ...
  useEffect(()=>{const c=new AbortController(),load=()=>fetchWallSpectrum(symbol,c.signal).then(data=>setLatest((data.rows||[]).at(-1)||{})).catch(()=>{});load();const timer=setInterval(load,5000);return()=>{c.abort();clearInterval(timer)}},[symbol]);
  ...
  return <section className="wall-strength-board"><header><div><span>WALL INTELLIGENCE · STRENGTH BOARD</span><h3>{symbol} · current calculated walls by relative strength</h3></div><b>LIVE · 5 SEC</b></header><div className="wall-strength-scroll"><table><thead><tr><th>WALL VARIABLE</th>{tiers.map(tier=><th className={tier.toLowerCase()} key={tier}>{tier}</th>)}</tr></thead><tbody>{items.map(([label,key])=>{const wall=walls[key]||{},tier=String(wall.tier||"WEAKEST").toUpperCase();return <tr key={key}><td><b className={key.toLowerCase()}>{label}</b><small>Strike {number(wall.strike).toFixed(2)}</small></td>{tiers.map(rank=><td className={rank===tier?`active ${rank.toLowerCase()}`:""} key={rank}>{rank===tier?<><b>{number(wall.strike).toFixed(2)}</b><small>EST. {compact(wall.dollar)}</small></>:"—"}</td>)}</tr>})}</tbody></table></div><footer>Tier compares the current point-in-time wall with its observed session history; the displayed amount is estimated GEX from delayed OI × Greeks.</footer></section>;
}
