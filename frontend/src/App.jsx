...
const logDate = value => {const date=new Date(value);return Number.isNaN(date.getTime())?"—":`${date.getFullYear()}-${String(date.getMonth()+1).padStart(2,"0")}-${String(date.getDate()).padStart(2,"0")}`};
const logTime = value => {const date=new Date(value);return Number.isNaN(date.getTime())?"—":`${String(date.getHours()).padStart(2,"0")}:${String(date.getMinutes()).padStart(2,"0")}:${String(date.getSeconds()).padStart(2,"0")}.${String(date.getMilliseconds()).padStart(3,"0")}`};
...
function deriveGammaDynamics(state,history=[]){
  ...
  const percentiles=Object.fromEntries(Object.entries(inputs).map(([name,value])=>[name,percentile(name,value)])),intensity=(percentiles.zomma+percentiles.color)/2,alignedUp=inputs.speed>1e-12&&inputs.gamma>1e-12,alignedDown=inputs.speed< -1e-12&&inputs.gamma< -1e-12,pressureMagnitude=(percentiles.speed+percentiles.gamma)/2,warmed=rows.length>=20,qualified=warmed&&intensity>=.65&&(alignedUp||alignedDown),decision=qualified?(alignedUp?"UP":"DOWN"):"NEUTRAL";
  return {available:true,qualified,decision,intensity,pressure:alignedUp?pressureMagnitude:alignedDown?-pressureMagnitude:0,history_points:rows.length,intensity_threshold:.65,inputs,percentiles,explanation:!warmed?`Building a relative baseline: ${rows.length}/20 observations.`:!(alignedUp||alignedDown)?"Gamma and Speed disagree, so signed curvature pressure is not confirmed.":intensity<.65?"Gamma and Speed align, but Zomma/Color intensity is below its rolling threshold.":`Gamma and Speed confirm ${alignedUp?"upward":"downward"} curvature pressure while Zomma and Color show elevated Gamma sensitivity.`};
}
...
function deriveGammaDynamicsEvents(history,state,symbol){
  ...
  const events=[];let prior="NEUTRAL",window=[];rows.forEach(row=>{window=[...window,row].slice(-100);const result=deriveGammaDynamics(row,window),decision=result.qualified?result.decision:"NEUTRAL";if(decision!=="NEUTRAL"&&decision!==prior)events.push({timestamp:row.timestamp,symbol:row.symbol??symbol,decision,intensity:result.intensity,pressure:result.pressure,...result.inputs});prior=decision});return events.reverse();
}
...
function OverviewDisclosure({id,title,description,children,defaultOpen=true}){
  const [open,setOpen]=useState(defaultOpen);
  return <details id={id} className="overview-disclosure overview-section" open={open} onToggle={event=>setOpen(event.currentTarget.open)}><summary><div><span>{open?"−":"+"}</span><div><b>{title}</b><small>{description}</small></div></div><em>{open?"COLLAPSE":"OPEN"}</em></summary><div className="overview-disclosure-body">{children}</div></details>;
}
...
function DraggableOverviewModule({id,index,dragged,dragOver,onDragStart,onDragOver,onDrop,onDragEnd,children}){
  ...
  return <div className={`draggable-overview-module ${dragged===id?"is-dragging":""} ${target?`is-drag-over drop-${dragOver.position}`:""}`} data-module-id={id} style={{order:index}}>
    <button type="button" className="module-drag-handle" draggable="true" onDragStart={event=>onDragStart(event,id)} onDragEnd={onDragEnd} aria-label={`Drag ${OVERVIEW_LABELS[id]} section to reorder`} title="Drag with your cursor to reorder this section"><span>⠿</span><b>DRAG TO REORDER</b><small>Position {index+1}</small></button>
...
function MomentumTriadChart({history=[],state,symbol,variant="triad"}){
  ...
  const valueFor=(row,name)=>greekValue(row,name),formatValue=value=>signedGreek(value),formatTime=value=>value?new Date(value).toLocaleTimeString([],{hour:"2-digit",minute:"2-digit",second:"2-digit"}):"--";
...
function OutcomeAttributionMini({system,data,symbol}){
  ...
  const rows=[...group.highest.map(item=>({...item,rankType:"HIGH / MFE"})),...group.lowest.map(item=>({...item,rankType:"LOW / MAE"}))];
  const source=[...group.highest,...group.lowest].find(Boolean)?.price_source??"WAITING";
  return <article className="panel outcome-attribution">
    <header className="panel-head"><div><span>OBSERVED OUTCOME ATTRIBUTION</span><h2>{SYSTEM_OUTCOME_LABELS[system]} · top 3 highs / bottom 3 lows</h2></div><div className="outcome-source"><b>{source.replaceAll("_"," ")}</b><small>{group.tracking} tracking · {group.total} total</small></div></header>
    <div className="outcome-method">30-minute observation window · HIGH uses maximum favorable excursion (MFE) · LOW uses maximum adverse excursion (MAE). Greek rankings are normalized associations, not causal proof.</div>
    <div className="outcome-scroll"><table><thead><tr><th>RANK</th><th>ALERT DATE</th><th>ALERT TIME · MS</th><th>CALL</th><th>QQQ</th><th>NQ</th><th>HIGH</th><th>LOW</th><th>MOVE</th><th>TIME TO EXTREME</th><th>STRONGEST</th><th>WEAKEST</th><th>DECAY</th><th>STATUS</th></tr></thead>
      <tbody>{rows.map((item,index)=>{const isHigh=item.rankType.startsWith("HIGH"),move=isHigh?number(item.favorable_points):number(item.adverse_points),extremeAt=isHigh?(item.direction==="UP"?item.highest_at:item.lowest_at):(item.direction==="UP"?item.lowest_at:item.highest_at),seconds=isHigh?(item.direction==="UP"?item.seconds_to_high:item.seconds_to_low):(item.direction==="UP"?item.seconds_to_low:item.seconds_to_high);return <tr key={`${item.id}-${item.rankType}`}><td><span className={`outcome-rank ${isHigh?"best":"worst"}`}>{item.rankType} #{index%3+1}</span></td><td>{logDate(item.alerted_at)}</td><td>{logTime(item.alerted_at)}</td><td><span className={`direction-pill ${String(item.direction).toLowerCase()}`}>{biasLabel(item.direction)}</span></td><td>{item.qqq_price==null?"—":number(item.qqq_price).toFixed(2)}</td><td>{item.nq_price==null?"NOT AVAILABLE":number(item.nq_price).toFixed(2)}</td><td>{number(item.highest_price).toFixed(2)}<small>{logDate(item.highest_at)} · {logTime(item.highest_at)}</small></td><td>{number(item.lowest_price).toFixed(2)}<small>{logDate(item.lowest_at)} · {logTime(item.lowest_at)}</small></td><td className={move>=0?"positive":"negative"}>{move>=0?"+":""}{move.toFixed(2)} pts</td><td>{duration(seconds)}<small>{logDate(extremeAt)} · {logTime(extremeAt)}</small></td><td>{pretty(item.strongest_greek??"—")}</td><td>{pretty(item.weakest_greek??"—")}</td><td>{pretty(item.decay_greek??"—")}</td><td>{item.status}</td></tr>})}</tbody>
    </table>{!rows.length&&<div className="empty-state">No qualified {SYSTEM_OUTCOME_LABELS[system]} decisions have started tracking yet. Start the live engine or run a replay; records are never fabricated.</div>}</div>
    <footer><span>Price source: {source.replaceAll("_"," ")}</span><span>{symbol==="QQQ"?"QQQ is observed directly.":"This free adapter does not claim an NQ price."}</span><span>Provider timestamps may be coarser than application observation milliseconds.</span></footer>
  </article>;
}
...
export default function Home() {
  ...
  const jumpTo=section=>{setActiveSection(section);setView("Overview");window.requestAnimationFrame(()=>window.requestAnimationFrame(()=>document.getElementById(section)?.scrollIntoView({behavior:"smooth",block:"start"})))};
  ...
  return <main className={`workspace focus-${focusTone}`}><header className="topbar"><div className="brand"><div className="brandmark"><span/><span/><span/></div><div><b>AXIOM</b><small>PRESSURE INTELLIGENCE</small></div></div><nav className="mode-switch"><button className={view!=="Historical Replay"?"active":""} onClick={()=>setView("Overview")}>Live engine</button><button className={view==="Historical Replay"?"active":""} onClick={()=>setView("Historical Replay")}>Training replay</button></nav><label className="section-jump"><span>Jump to</span><select value={activeSection} onChange={event=>jumpTo(event.target.value)}>{orderedOverviewSections.map(([label,id])=><option value={id} key={id}>{label}</option>)}</select></label><div className="header-actions status-cluster" aria-live="polite"><span className={`status-chip ${connected?"is-good":"is-bad"}`}><i/>API <b>{connected?"ONLINE":"OFFLINE"}</b></span><span className={`status-chip ${engine.running?"is-good":"is-idle"}`}><i/>ENGINE <b>{engine.running?"ON":"IDLE"}</b></span><span className={`status-chip ${dataFresh?"is-good":dataDelayed?"is-bad":"is-idle"}`}><i/>DATA <b>{dataFresh?`${stateAge}s`:dataDelayed?"STALE":"IDLE"}</b></span></div></header>
    <aside className="sidebar"><div className="side-top"><button className="navitem active" onClick={()=>jumpTo("decision")}><span className="icon">◫</span><span>Overview</span></button><nav className="overview-subnav" aria-label="Overview sections">{orderedOverviewSections.map(([label,section])=><button className={activeSection===section?"active":""} aria-current={activeSection===section?"location":undefined} key={section} onClick={()=>jumpTo(section)}><i/><span>{label}</span></button>)}</nav></div><div className="side-bottom"><div className={`system-health ${system?.database_connected?"is-good":"is-bad"}`}><span><i/>{system?.database_connected?"System healthy":"System degraded"}</span><small>v{config?.version??"—"} · Render</small></div></div></aside>
