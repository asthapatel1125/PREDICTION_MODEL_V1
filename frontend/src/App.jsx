...
function deriveGammaDynamicsEvents(history,state,symbol){
  ...
  const events=[];let prior="NEUTRAL",window=[];rows.forEach(row=>{window=[...window,row].slice(-100);const result=deriveGammaDynamics(row,window),decision=result.qualified?result.decision:"NEUTRAL";if(decision!=="NEUTRAL"&&decision!==prior)events.push({timestamp:row.timestamp,symbol:row.symbol??symbol,price:number(row?.supporting_indicators?.price,NaN),decision,intensity:result.intensity,pressure:result.pressure,...result.inputs});prior=decision});return events.reverse();
}
...
function OutcomeAttributionMini({system,data,symbol}){
  ...
  return <article className="panel outcome-attribution">
    ...
    <footer><span>Price source: {source.replaceAll("_"," ")}</span><span>Each Call ID is Eastern timestamp-based: YYYY/MM/DD/HH/MM/SS/MS-system-symbol.</span><span>Click a Call ID to copy it, or copy the full tab-separated table.</span></footer>
  </article>;
}
