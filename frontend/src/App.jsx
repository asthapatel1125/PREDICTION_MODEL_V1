...
function GammaDynamicsLog({history,state,symbol,calls=[]}){
  ...
  const copySummaries=async()=>{
    ...
    const rows=events.map(event=>{const call=linkedCall(event),view=visualState(call),finished=Boolean(call&&callOutcome(call).closed),high=call?number(finished?call.highest_price:call.dynamic_high,call.highest_price):number(event.price,NaN),low=call?number(finished?call.lowest_price:call.dynamic_low,call.lowest_price):number(event.price,NaN),datum=number(call?.entry_price,event.price),currentPrice=call?number(finished?(call.final_price??call.current_price):(call.current_price??call.minute_bars?.at(-1)?.close),datum):number(event.price,NaN);return [event.id,view.label,event.decision,`${logDate(event.timestamp)} ${logTime(event.timestamp)}`,datum.toFixed(4),Number.isFinite(high)?high.toFixed(4):"—",Number.isFinite(high)?`${high-datum>=0?"+":""}${(high-datum).toFixed(4)} pts`:"—",finished?duration(call.seconds_to_high):"Tracking",Number.isFinite(low)?low.toFixed(4):"—",Number.isFinite(low)?`${low-datum>=0?"+":""}${(low-datum).toFixed(4)} pts`:"—",finished?duration(call.seconds_to_low):"Tracking",Number.isFinite(currentPrice)?currentPrice.toFixed(4):"—",Number.isFinite(currentPrice)?`${currentPrice-datum>=0?"+":""}${(currentPrice-datum).toFixed(4)} pts`:"—"].map(clean)});
  ...
  return <article className="panel gamma-dynamics-log">
    ...
    <div className="gamma-log-scroll"><table>
      ...
      <tbody>{events.map((event,index)=>{
        const call=linkedCall(event),expired=call&&callDeadlinePassed(call),snapshot=call?deadlineSnapshot(call):{},outcome=callOutcome(call),view=visualState(call),finished=outcome.closed;
        const livePrice=call?(finished?number(call.final_price??call.current_price??snapshot.price,NaN):number(call.current_price??call.minute_bars?.at(-1)?.close??call.entry_price,NaN)):event.price;
        const high=call?(finished?number(call.highest_price,snapshot.high):number(call.dynamic_high,call.highest_price)):event.price,low=call?(finished?number(call.lowest_price,snapshot.low):number(call.dynamic_low,call.lowest_price)):event.price;
...
function mergeSystemOutcome(current,record){
  if(!record?.system)return current;
  const systems=current?.systems??{},group=systems[record.system]??{highest:[],lowest:[],calls:[],tracking:0,total:0};
  const calls=[record,...(group.calls??[]).filter(call=>call.id!==record.id)].sort((a,b)=>new Date(b.alerted_at)-new Date(a.alerted_at)).slice(0,100);
  return {...current,symbol:record.symbol??current?.symbol,systems:{...systems,[record.system]:{...group,calls,tracking:calls.filter(call=>call.status==="TRACKING").length,total:Math.max(number(group.total),calls.length)}}};
}
...
export default function Home() {
  ...
  useEffect(()=>subscribeToEvents(message=>{if(message.topic==="market_state"){setDashboard(current=>({...current,state:message.payload,history:[...(current.history??[]),message.payload].slice(-120)}));setChartHistory(current=>[...current.filter(row=>row.timestamp!==message.payload.timestamp),message.payload].slice(-5000))}if(message.topic==="alert")setDashboard(current=>({...current,alerts:[toDashboardAlert(message.payload),...(current.alerts??[])].slice(0,100)}));if(message.topic==="outcome")setDashboard(current=>({...current,alerts:(current.alerts??[]).map(alert=>alert.id===message.payload.alert_id?{...alert,result:number(message.payload.precision)>=.7?"SUCCESS":"FAILURE",precision:number(message.payload.precision).toFixed(2)}:alert)}));if(message.topic==="system_outcome")setAttribution(current=>mergeSystemOutcome(current,message.payload));if(message.topic==="engine_status"){setDashboard(current=>({...current,engine:message.payload}));setSystem(current=>current?{...current,engine:message.payload}:current)}if(message.topic==="system_event")setSystem(current=>current?{...current,events:[message.payload,...(current.events??[])].slice(0,25)}:current);if(message.topic==="replay_status")setReplay(message.payload)},setConnected),[]);
