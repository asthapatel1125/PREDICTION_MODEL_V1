...
import {
  fetchChart, fetchConfiguration, fetchDashboard, fetchDynamicsSessionHistory, fetchInstruments, fetchOutcomeAttribution, fetchOutcomeCall, fetchReplay, fetchSystem,
...
function GammaDynamicsV2FlowLog({history=[],state,symbol}){
  ...
  const rows=[...new Map(source.map(row=>[row.timestamp,row])).values()].sort((a,b)=>new Date(b.timestamp)-new Date(a.timestamp)).map(row=>({timestamp:row.timestamp,model:row.gamma_dynamics_v2,metrics:row.gamma_dynamics_v2?.chain_metrics??{}})).filter(row=>row.model?.qualified&&Number(row.metrics.chain_available)>0).slice(0,90);
  const [scrollSize,setScrollSize]=useState({width:0,height:0});
...
export default function Home() {
  ...
  useEffect(()=>{const controller=new AbortController();fetchDynamicsSessionHistory(symbol,sessionDate,controller.signal).then(result=>setChartHistory(result.rows??[])).catch(error=>{if(error.name!=="AbortError")setChartHistory([])});return()=>controller.abort()},[symbol,sessionDate]);
  ...
  useEffect(()=>subscribeToEvents(message=>{if(message.topic==="market_state"){setDashboard(current=>({...current,state:message.payload,history:[...(current.history??[]),message.payload].slice(-120)}));setChartHistory(current=>[...current.filter(row=>row.timestamp!==message.payload.timestamp),message.payload].slice(-10000))}if(message.topic==="alert")setDashboard(current=>({...current,alerts:[toDashboardAlert(message.payload),...(current.alerts??[])].slice(0,100)}));if(message.topic==="outcome")setDashboard(current=>({...current,alerts:(current.alerts??[]).map(alert=>alert.id===message.payload.alert_id?{...alert,result:number(message.payload.precision)>=.7?"SUCCESS":"FAILURE",precision:number(message.payload.precision).toFixed(2)}:alert)}));if(message.topic==="engine_status"){setDashboard(current=>({...current,engine:message.payload}));setSystem(current=>current?{...current,engine:message.payload}:current)}if(message.topic==="system_event")setSystem(current=>current?{...current,events:[message.payload,...(current.events??[])].slice(0,25)}:current);if(message.topic==="replay_status")setReplay(message.payload)},setConnected),[]);
