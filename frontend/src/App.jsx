...
function ZoneIntelligenceFixed({symbol}){
  ...
  useEffect(()=>{const controller=new AbortController();const load=()=>Promise.all([fetchWallSpectrum(symbol,controller.signal),fetchWallBreaks(symbol,controller.signal),fetchWallSummaryHistory(symbol,controller.signal),fetchWallDayLevels(symbol,null,controller.signal)]).then(([s,b,h,d])=>{setSpectrum(s.rows||[]);setBreaks(b.rows||[]);setSummaries(h.rows||[]);setDayRows(d.rows||[]);setPhaseAnchors(d.phase_anchors||[])}).catch(()=>{});load();const id=window.setInterval(load,5000);return()=>{controller.abort();clearInterval(id)}},[symbol]);
