...
const chartTime = value => value&&!Number.isNaN(new Date(value).getTime())?`${new Date(value).toLocaleTimeString("en-US",{timeZone:EASTERN_TZ,hour12:true,hour:"2-digit",minute:"2-digit",second:"2-digit"})} EST`:"—";
...
function deriveGammaDynamics(state,history=[]){
  ...
  const percentiles=Object.fromEntries(Object.entries(inputs).map(([name,value])=>[name,percentile(name,value)])),normalized=Object.fromEntries(Object.entries(inputs).map(([name,value])=>[name,scaled(name,value)])),intensity=(percentiles.zomma+percentiles.color)/2,gammaActive=Math.abs(inputs.gamma)>1e-12,alignedUp=inputs.speed>1e-12&&gammaActive,alignedDown=inputs.speed< -1e-12&&gammaActive,pressureMagnitude=(percentiles.speed+percentiles.gamma)/2,warmed=rows.length>=20,qualified=warmed&&intensity>=.65&&(alignedUp||alignedDown),decision=qualified?(alignedUp?"UP":"DOWN"):"NEUTRAL",ideal_ranges={zomma:"Magnitude percentile >= 65% · IV-to-Gamma sensitivity",color:"Magnitude percentile >= 65% · time-to-Gamma sensitivity",speed:"Non-zero and signed with call direction",gamma:"Non-zero curvature base · >= 50% preferred"};
...
const gammaGreekIdeal=(name,model)=>{
  const value=number(model?.inputs?.[name]),rank=number(model?.percentiles?.[name]);
  if(number(model?.history_points)<20)return false;
  if(name==="speed")return Math.abs(value)>1e-12&&(model?.decision==="NEUTRAL"||(model?.decision==="UP"&&value>0)||(model?.decision==="DOWN"&&value<0));
  if(name==="gamma")return Math.abs(value)>1e-12&&rank>=.5;
  return rank>=.65;
};
...
function GammaDynamicsChart({history=[],state,symbol,deltaMode=false,gammaVersion=1}){
  ...
  return <ChartShell expanded={expanded} setExpanded={setExpanded} className={`gamma-dynamics-history-chart gamma-dynamics-chart ${deltaMode?"delta-dynamics-chart":v2?"gamma-dynamics-v2-chart":""}`}>
    <div className="greek-chart-header"><div><span>{deltaMode?"DELTA DYNAMICS HISTORY":`GAMMA DYNAMICS ${gammaVersion}.0 HISTORY · NORMALIZED LIVE STREAM`}</span><h2>{symbol} · {deltaMode?"Ultima / Zomma / Gamma / Speed / Color / Delta":series.map(item=>pretty(item[0])).join(" / ")}</h2></div><div className="greek-header-actions"><ChartTimeControls {...{intervalSeconds,setIntervalSeconds,isLive:viewport.isLive,setOffset:viewport.setOffset,expanded,setExpanded,zoom,onZoom:zoomChart}}/></div></div>
