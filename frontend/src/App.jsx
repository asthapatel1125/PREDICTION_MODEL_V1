...
import {
  fetchChart, fetchConfiguration, fetchDashboard, fetchInstruments, fetchOutcomeAttribution, fetchOutcomeCall, fetchReplay, fetchStateHistory, fetchSystem,
...
function gammaV2GateSummary(state){
  ...
  if(!entries.length)return {summary:"WAITING FOR LIVE DATA",detail:"WAITING ON · CHAIN SNAPSHOT",tone:"waiting"};
  if(model?.qualified)return {summary:`${passed} / ${entries.length} PASS`,detail:`QUALIFIED · ${biasLabel(model.decision)}`,tone:"qualified"};
  ...
  return {summary:`${passed} / ${entries.length} PASS`,detail:`WAITING ON · ${pretty(blocker).toUpperCase()}`,tone:"waiting"};
}

function SystemScorecardRow({system,label,calls,state}){
  ...
  const gate=system==="GAMMA_DYNAMICS_V2"?gammaV2GateSummary(state):null;
  return <tr className={`system-scorecard-row ${scorecardClass}`}><td className="scorecard-system"><b>{label}</b><small>{closed} CLOSED</small></td><td className="scorecard-rate">{successRate===null?"—":pct(successRate)}</td><td className="scorecard-success">{counts.succeeded}</td><td className="scorecard-failure">{counts.failed}</td><td className="scorecard-success">{counts.succeeding}</td><td className="scorecard-failure">{counts.failing}</td><td className="scorecard-targets">{speedBuckets.map(bucket=><span key={bucket.label}><b>{bucket.count}</b><small>{bucket.label}</small></span>)}</td><td className="scorecard-favorable-cell">{averageFavorable===null?"—":`+${averageFavorable.toFixed(2)} PT`}</td><td>{medianWin===null?"—":duration(medianWin)}</td><td className={`scorecard-gate-cell ${gate?.tone??""}`}>{gate?<><small>{gate.summary}</small><b>{gate.detail}</b></>:"—"}</td></tr>;
}
...
function GammaDynamicsChart({history=[],state,symbol,deltaMode=false,gammaVersion=1}){
  ...
  const [intervalSeconds,setIntervalSeconds]=useState(5),[expanded,setExpanded]=useState(false),[zoom,setZoom]=useState(1),[cursor,setCursor]=useState(null),[showMinuteStream,setShowMinuteStream]=useState(false),[streamCopied,setStreamCopied]=useState(false);
  ...
  return <ChartShell expanded={expanded} setExpanded={setExpanded} className={`gamma-dynamics-history-chart gamma-dynamics-chart ${deltaMode?"delta-dynamics-chart":v2?"gamma-dynamics-v2-chart":""}`}>
    <div className="greek-chart-header"><div><span>{deltaMode?"DELTA DYNAMICS HISTORY":`GAMMA DYNAMICS ${gammaVersion}.0 HISTORY · NORMALIZED LIVE STREAM`}</span><h2>{symbol} · {deltaMode?"Ultima / Zomma / Gamma / Speed / Color / Delta":series.map(item=>pretty(item[0])).join(" / ")}</h2></div><div className="greek-header-actions">{deltaMode&&<button type="button" className="copy-table delta-minute-stream-button" onClick={copyMinuteStream} disabled={!minuteStream.length}>{streamCopied?"✓ COPIED":"COPY MINUTE STREAM"}</button>}<ChartTimeControls {...{intervalSeconds,setIntervalSeconds,isLive:viewport.isLive,setOffset:viewport.setOffset,expanded,setExpanded,zoom,onZoom:zoomChart}}/></div></div>
...
function GammaEventGraphModal({event,call,version=1,onClose}){
  ...
  return createPortal(<div className="event-graph-backdrop" role="dialog" aria-modal="true" aria-label="Gamma Dynamics event details" onPointerDown={pointerEvent=>{if(pointerEvent.target===pointerEvent.currentTarget)onClose()}}><section className="event-graph-dialog event-call-dialog"><header><div><span>GAMMA DYNAMICS {version}.0 · EVENT DETAILS</span><b>{call.symbol} · {biasLabel(call.direction)} · {logDate(call.alerted_at)} {logTime(call.alerted_at)}</b></div><button type="button" onClick={onClose}>CLOSE</button></header><div className="event-call-dialog-body"><GammaEventLogSummary event={event} call={call}/>{version===2&&<GammaDynamicsV2EventData event={event} call={call}/>}<FiftyPointOutcomeCard call={call} system={version===2?"GAMMA_DYNAMICS_V2":"GAMMA_DYNAMICS"}/></div></section></div>,document.body);
}
