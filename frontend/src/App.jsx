...
function GammaDynamicsLog({history,state,symbol,calls=[],version=1}){
  ...
  return <article className={`panel gamma-dynamics-log ${v2?"gamma-dynamics-log-v2":""}`}>
    ...
    {!v2&&graphEvent&&<GammaEventGraphModal {...graphEvent} onClose={()=>setExpandedDashboardId(null)}/>}</article>;
}
...
function GammaEventGraphModal({event,call,onClose}){
  ...
  return createPortal(<div className="event-graph-backdrop" role="dialog" aria-modal="true" aria-label="Gamma Dynamics event details" onPointerDown={pointerEvent=>{if(pointerEvent.target===pointerEvent.currentTarget)onClose()}}><section className="event-graph-dialog event-call-dialog"><header><div><span>GAMMA DYNAMICS 1.0 · EVENT DETAILS</span><b>{call.symbol} · {biasLabel(call.direction)} · {logDate(call.alerted_at)} {logTime(call.alerted_at)}</b></div><button type="button" onClick={onClose}>CLOSE</button></header><div className="event-call-dialog-body"><div className="event-popup-summary"><table><thead><tr><th>DIRECTION</th><th>CALL STATE</th><th>STREAM DURATION</th><th>HIGH CHANGE</th><th>TIME · EST</th><th>DATE · EST</th><th>SOURCE</th><th>DATUM</th><th>TARGET</th><th>HIGH</th><th>LOW</th><th>CURRENT / FINAL</th><th>INTENSITY</th><th>PRESSURE</th>{GAMMA_DYNAMICS_GREEKS.map(name=><th key={name}>{name.toUpperCase()} · RAW</th>)}</tr></thead><tbody><tr><td>{biasLabel(call.direction)}</td><td>{view.label}</td><td>{callStreamDuration(call)}</td><td>{change(high)}</td><td>{logTime(call.alerted_at)}</td><td>{logDate(call.alerted_at)}</td><td>{call.symbol}</td><td>{datum.toFixed(4)}</td><td>{number(call.target_price,gammaReachFromDatum(datum,call.direction,call.alerted_at)).toFixed(4)}</td><td>{Number.isFinite(high)?high.toFixed(4):"—"}</td><td>{Number.isFinite(low)?low.toFixed(4):"—"}</td><td>{Number.isFinite(current)?current.toFixed(4):"—"}</td><td>{pct(event.intensity)}</td><td>{Number.isFinite(event.pressure)?`${event.pressure>0?"+":""}${event.pressure.toFixed(2)}`:"—"}</td>{GAMMA_DYNAMICS_GREEKS.map(name=><td key={name}>{signedGreek(call.greek_values_at_signal?.[name]??event[name])}</td>)}</tr></tbody></table></div><FiftyPointOutcomeCard call={call} system="GAMMA_DYNAMICS"/></div></section></div>,document.body);
}
