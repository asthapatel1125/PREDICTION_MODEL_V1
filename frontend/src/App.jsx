...
function CustomGreekChart({chart,index,history,state,symbol,onChange,onRemove,canRemove}){
  ...
  const dims={width:1200,height:Math.max(expanded?700:380,series.length*(expanded?132:105)+62),left:138,right:30,top:20,bottom:58},plotWidth=dims.width-dims.left-dims.right,plotHeight=dims.height-dims.top-dims.bottom,laneHeight=plotHeight/Math.max(series.length,1);
  ...
  return <ChartShell expanded={expanded} setExpanded={setExpanded} className="custom-greek-chart">
    ...
    <div className="greek-chart-stage lane-chart-stage" style={{height:Math.min(dims.height,expanded?760:560)}} onWheel={event=>{event.preventDefault();if(event.ctrlKey||event.metaKey)zoomChart(event.deltaY<0?.5:-.5);else viewport.move(event.deltaY>0?10:-10)}} onPointerDown={onPointerDown} onPointerMove={onPointerMove} onPointerUp={onPointerUp} onPointerLeave={()=>{drag.current=null;setCursor(null)}}>
