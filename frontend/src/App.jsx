...
import {
  fetchChart, fetchConfiguration, fetchDashboard, fetchDeltaDynamicsHistory, fetchInstruments, fetchOutcomeAttribution, fetchOutcomeCall, fetchReplay, fetchStateHistory, fetchSystem, startLiveEngine,
...
function GammaDynamicsModule({state,history,symbol,engine,version=1}){
  ...
  const normalizedSpeed=number(quartet.normalized?.speed),normalizedGamma=number(quartet.normalized?.gamma),aligned=v2?((normalizedSpeed>=GAMMA_DYNAMICS_V2_IDEALS.speed.threshold&&normalizedGamma>=GAMMA_DYNAMICS_V2_IDEALS.gamma.threshold)||(normalizedSpeed<=-GAMMA_DYNAMICS_V2_IDEALS.speed.threshold&&normalizedGamma<=-GAMMA_DYNAMICS_V2_IDEALS.gamma.threshold)):Math.abs(number(quartet.inputs?.speed))>1e-12&&Math.abs(number(quartet.inputs?.gamma))>1e-12,warmed=number(quartet.history_points)>=20,intensityPassed=number(quartet.intensity)>=number(quartet.intensity_threshold,.65);
...
function DeltaDynamicsStreamAudit({symbol}){
  const [filters,setFilters]=useState({startDate:"2026-08-04",endDate:"2026-08-04",startTime:"09:00",endTime:"17:00"});
  const [applied,setApplied]=useState(filters),[rows,setRows]=useState([]),[source,setSource]=useState(""),[loading,setLoading]=useState(true),[error,setError]=useState(""),[copied,setCopied]=useState(false);
  useEffect(()=>{const controller=new AbortController();setLoading(true);setError("");fetchDeltaDynamicsHistory(symbol,applied,controller.signal).then(result=>{setRows(result.rows??[]);setSource(result.source??"")}).catch(reason=>{if(reason.name!=="AbortError"){setRows([]);setSource("");setError(reason.message||"Unable to load the ThetaData stream.")}}).finally(()=>{if(!controller.signal.aborted)setLoading(false)});return()=>controller.abort()},[symbol,applied]);
  const audited=useMemo(()=>rows.map(row=>{const model=deriveSixGreekDynamics(row,[]),zone=row.zone_intelligence??{},inputs=Object.fromEntries(SIX_GREEKS.map(name=>[name,greekValue(row,name)]));return {timestamp:row.timestamp,symbol:row.symbol??symbol,inputs,normalized:zone.normalized??model.scaled??{},zone:zone.zone??model.zone??"NO_ZONE",score:number(zone.score,model.score),qualified:Boolean(zone.qualified??model.qualified),direction:zone.direction??model.decision??"NEUTRAL",historyPoints:number(zone.history_points,model.history_points)}}),[rows,symbol]);
  const update=(name,value)=>setFilters(current=>({...current,[name]:value}));
  const apply=()=>setApplied({...filters});
  const copy=async()=>{const headers=["Date · Eastern","Time · Eastern","Symbol","Aligned","Zone","Direction","Zone Match","History Points",...SIX_GREEKS.map(name=>`${name.toUpperCase()} Raw`),...SIX_GREEKS.map(name=>`${name.toUpperCase()} Normalized`)];const values=audited.map(row=>[logDate(row.timestamp),logTime(row.timestamp),row.symbol,row.qualified?"YES":"NO",pretty(row.zone),biasLabel(row.direction),row.score,row.historyPoints,...SIX_GREEKS.map(name=>Number.isFinite(row.inputs[name])?row.inputs[name]:""),...SIX_GREEKS.map(name=>Number.isFinite(Number(row.normalized[name]))?Number(row.normalized[name]):"")]);try{await navigator.clipboard.writeText(exportTsv(`DELTA DYNAMICS STREAM AUDIT · ${symbol} · EASTERN TIME`,headers,values));setCopied(true);setTimeout(()=>setCopied(false),1200)}catch{setCopied(false)}};
  return <article className="panel delta-stream-audit"><header className="panel-head"><div><span>DELTA DYNAMICS · HISTORICAL STREAM AUDIT</span><h2>Every persisted Greek snapshot · green when the active zone aligned</h2></div><div className="gamma-log-actions"><button type="button" className="copy-table" onClick={copy} disabled={!audited.length}>{copied?"✓ COPIED":"COPY TABLE"}</button><b>{audited.length} ROWS</b></div></header>
    <div className="delta-audit-controls"><label>FROM DATE<input type="date" value={filters.startDate} onChange={event=>update("startDate",event.target.value)}/></label><label>TO DATE<input type="date" value={filters.endDate} onChange={event=>update("endDate",event.target.value)}/></label><label>FROM TIME<input type="time" value={filters.startTime} onChange={event=>update("startTime",event.target.value)}/></label><label>TO TIME<input type="time" value={filters.endTime} onChange={event=>update("endTime",event.target.value)}/></label><button type="button" onClick={apply}>APPLY FILTER</button></div>
    <div className="delta-audit-note"><span><b>GREEN</b> = backend zone qualification passed</span><span>Requires at least 20 observations and ≥80% of the numerical gates for the active Eastern-time zone.</span><span>Source: {source?pretty(source):"loading"}</span></div>
    <div className="delta-audit-scroll"><table><thead><tr><th>DATE · EASTERN</th><th>TIME · EASTERN</th><th>SYMBOL</th><th>ALIGNED</th><th>ZONE</th><th>DIRECTION</th><th>ZONE MATCH</th><th>HISTORY POINTS</th>{SIX_GREEKS.map(name=><th key={`${name}-raw`}>{name.toUpperCase()} · RAW</th>)}{SIX_GREEKS.map(name=><th key={`${name}-normalized`}>{name.toUpperCase()} · NORMALIZED</th>)}</tr></thead><tbody>{audited.map((row,index)=><tr className={row.qualified?"aligned":""} key={`${row.timestamp}-${index}`}><td>{logDate(row.timestamp)}</td><td>{logTime(row.timestamp)}</td><td>{row.symbol}</td><td><span className={`delta-alignment-badge ${row.qualified?"yes":"no"}`}>{row.qualified?"ALIGNED":"NO"}</span></td><td>{pretty(row.zone)}</td><td>{biasLabel(row.direction)}</td><td>{pct(row.score)}</td><td>{row.historyPoints}</td>{SIX_GREEKS.map(name=><td key={`${name}-raw`}>{signedGreek(row.inputs[name])}</td>)}{SIX_GREEKS.map(name=><td key={`${name}-normalized`}>{Number.isFinite(Number(row.normalized[name]))?`${number(row.normalized[name])>=0?"+":""}${number(row.normalized[name]).toFixed(3)}`:"—"}</td>)}</tr>)}</tbody></table>{loading&&<div className="delta-audit-empty">Loading persisted stream…</div>}{!loading&&error&&<div className="delta-audit-empty error">{error}</div>}{!loading&&!error&&!audited.length&&<div className="delta-audit-empty">No persisted snapshots were found in this Eastern-time range.</div>}</div>
  </article>;
}
