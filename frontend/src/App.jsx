...
import {
  fetchChart, fetchConfiguration, fetchDashboard, fetchDynamicsHistory, fetchInstruments, fetchOutcomeAttribution, fetchOutcomeCall, fetchReplay, fetchSystem, fetchWallSpectrum, fetchWallBreaks, fetchWallDealerFlow, fetchWallSummaryHistory, fetchWallDayLevels,
...
const DEFAULT_MODULE_ORDER=["wall-intelligence","gamma-dynamics","gamma-dynamics-v2","six-greek-dynamics","forecast","score-modules","greek-orders","live-alerts"];
...
function LegacyDailyWallLevelsChart({rows,phaseAnchors=[]}){
  ...
  const colors={spot:"#e6edf3",CALL_WALL:"#00d084",PUT_WALL:"#ff4f69",ZERO_GAMMA:"#b56cff",SUPPORT:"#48d597",RESISTANCE:"#ff8b75",volume:"#4ca8ff"};
...
function LegacyDailyWallLevelsChartV2({rows,phaseAnchors=[]}){
  ...
  if(!minutes.length)return <p className="wall-empty-state">Waiting for point-in-time wall observations from the current session.</p>;
  const levels=["CALL_WALL","PUT_WALL","ZERO_GAMMA","SUPPORT","RESISTANCE"],colors={spot:"#e6edf3",CALL_WALL:"#00d084",PUT_WALL:"#ff4f69",ZERO_GAMMA:"#b56cff",SUPPORT:"#ff61b6",RESISTANCE:"#ff8b75"},value=(row,key)=>key==="spot"?number(row.spot):number(row.walls?.[key]?.strike),anchor=(row,key)=>number(phaseAnchors.find(item=>{const at=new Date(row.timestamp).getTime();return at>=new Date(item.start).getTime()&&at<new Date(item.end).getTime()})?.levels?.[key]??value(row,key)),all=[...minutes.map(row=>value(row,"spot")),...levels.flatMap(key=>minutes.map(row=>anchor(row,key)))],baseLow=Math.min(...all),baseHigh=Math.max(...all),baseRange=Math.max(baseHigh-baseLow,.01),focus=hover===null?(baseLow+baseHigh)/2:value(minutes[hover],"spot"),range=baseRange/yZoom,low=focus-range/2,high=focus+range/2;
...
function DailyWallLevelsChart({rows,phaseAnchors=[]}){
  const [windowName,setWindowName]=useState("1H"),[hover,setHover]=useState(null),[xZoom,setXZoom]=useState(9),[yZoom,setYZoom]=useState(1),scrollRef=useRef(null);
  const allRows=useMemo(()=>{const byMinute=new Map();rows.forEach(row=>{const key=new Date(row.timestamp).toISOString().slice(0,16);byMinute.set(key,row)});return [...byMinute.values()]},[rows]);
  const seconds={"5M":300,"15M":900,"1H":3600,"ALL":Infinity}[windowName],latestAt=new Date(allRows.at(-1)?.timestamp||0).getTime(),viewRows=seconds===Infinity?allRows:allRows.filter(row=>new Date(row.timestamp).getTime()>=latestAt-seconds*1000);
...
function DealerFlowLanes({rows,traces,colors,valueFor}){
...
function ZoneIntelligenceFixed({symbol}){
  const [spectrum,setSpectrum]=useState([]),[breaks,setBreaks]=useState([]),[summaries,setSummaries]=useState([]),[dayRows,setDayRows]=useState([]),[phaseAnchors,setPhaseAnchors]=useState([]);
  useEffect(()=>{const controller=new AbortController();const load=async()=>{const [spectrumResult,breakResult,summaryResult,dayResult]=await Promise.allSettled([fetchWallSpectrum(symbol,controller.signal),fetchWallBreaks(symbol,controller.signal),fetchWallSummaryHistory(symbol,controller.signal),fetchWallDayLevels(symbol,null,controller.signal)]);if(spectrumResult.status==="fulfilled")setSpectrum(spectrumResult.value.rows||[]);if(breakResult.status==="fulfilled")setBreaks(breakResult.value.rows||[]);if(summaryResult.status==="fulfilled")setSummaries(summaryResult.value.rows||[]);if(dayResult.status==="fulfilled"){setDayRows(dayResult.value.rows||[]);setPhaseAnchors(dayResult.value.phase_anchors||[])};};load().catch(()=>{});const id=window.setInterval(()=>{load().catch(()=>{})},5000);return()=>{controller.abort();clearInterval(id)}},[symbol]);
  const latest=spectrum.at(-1)||dayRows.at(-1)||{},walls=latest.walls||{},compact=value=>{const n=number(value);return Math.abs(n)>=1e9?`${(n/1e9).toFixed(2)}B`:Math.abs(n)>=1e6?`${(n/1e6).toFixed(1)}M`:n.toFixed(2)};
...
function WallIntelligenceWorkspace({symbol}){
  ...
  const rows=spectrum.slice(-360),colors={"QQQ Price":"#e6edf3","Call Wall":"#00d084","Put Wall":"#ff3b30","Zero Gamma":"#a855f7","Support":"#00a86b","Resistance":"#ff6b63","DealerFlow":"#00d084","DEX":"#58a6ff","VolHack":"#ffd60a","Pos Inventory":"#5ce1b8","Neg Inventory":"#ff7382","TW GEX":"#a855f7","GEX Density":"#ff8a00","Spoof Score":"#ffd60a","Edge":"#00d084","Liquidity":"#8b949e","VIX":"#ff8a00"};
...
function WallIntelligenceWorkspaceV2({symbol}){
  ...
  useEffect(()=>{const c=new AbortController(),load=()=>Promise.all([fetchWallSpectrum(symbol,c.signal),fetchWallBreaks(symbol,c.signal),fetchWallSummaryHistory(symbol,c.signal)]).then(([s,b,h])=>{setSpectrum(s.rows||[]);setBreaks(b.rows||[]);setSummaries(h.rows||[])}).catch(()=>{});load();const t=setInterval(load,5000);return()=>{c.abort();clearInterval(t)}},[symbol]);
  const rows=spectrum.slice(-360),summary=summaries.at(-1),colors={"QQQ Price":"#e6edf3","Call Wall":"#00d084","Put Wall":"#ff3b30","Zero Gamma":"#a855f7","Support":"#00a86b","Resistance":"#ff6b63","DealerFlow":"#00d084","DEX":"#58a6ff","VolHack":"#ffd60a","Pos Inventory":"#5ce1b8","Neg Inventory":"#ff7382","TW GEX":"#a855f7","GEX Density":"#ff8a00","Spoof Score":"#ffd60a","Edge":"#00d084","Liquidity":"#8b949e","VIX":"#ff8a00"};
...
function WallStrengthDashboard({symbol}){
  const [latest,setLatest]=useState({}),[phaseAnchors,setPhaseAnchors]=useState([]);
  useEffect(()=>{const c=new AbortController(),load=()=>Promise.all([fetchWallSpectrum(symbol,c.signal),fetchWallDayLevels(symbol,null,c.signal)]).then(([s,d])=>{setLatest((s.rows||[]).at(-1)||{});setPhaseAnchors(d.phase_anchors||[])}).catch(()=>{});load();const timer=setInterval(load,5000);return()=>{c.abort();clearInterval(timer)}},[symbol]);
  const compact=value=>{const n=number(value);return Math.abs(n)>=1e9?`${(n/1e9).toFixed(2)}B`:Math.abs(n)>=1e6?`${(n/1e6).toFixed(1)}M`:n.toFixed(2)},tiers=["STRONGEST","STRONG","NORMAL","WEAK","WEAKEST"],walls=latest.walls||{},items=[["CALL WALL","CALL_WALL"],["PUT WALL","PUT_WALL"],["ZERO GAMMA","ZERO_GAMMA"],["SUPPORT","SUPPORT"],["RESISTANCE","RESISTANCE"]];
  const activeAnchor=phaseAnchors.find(anchor=>{const at=new Date(latest.timestamp||Date.now()).getTime();return at>=new Date(anchor.start).getTime()&&at<new Date(anchor.end).getTime()});
  return <section className="wall-strength-board"><header><div><span>WALL INTELLIGENCE · STRENGTH BOARD</span><h3>{symbol} · market-phase levels by live relative strength</h3></div><b>{activeAnchor?`${activeAnchor.phase.replace("_"," ")} ANCHOR`:"LIVE · 5 SEC"}</b></header><div className="wall-strength-scroll"><table><thead><tr><th className="wall-variable">WALL VARIABLE</th>{tiers.map(tier=><th className={tier.toLowerCase()} key={tier}>{tier}</th>)}</tr></thead><tbody>{items.map(([label,key])=>{const wall=walls[key]||{},tier=String(wall.tier||"WEAKEST").toUpperCase(),strike=number(activeAnchor?.levels?.[key]??wall.strike);return <tr key={key}><td className="wall-variable"><b className={key.toLowerCase()}>{label}</b><small>{activeAnchor?`${activeAnchor.phase.replace("_"," ")} ${strike.toFixed(2)}`:`Strike ${strike.toFixed(2)}`}</small></td>{tiers.map(rank=><td className={rank===tier?`active ${rank.toLowerCase()}`:""} key={rank}>{rank===tier?<><b>{strike.toFixed(2)}</b><small>{activeAnchor?"PHASE ANCHOR · ":""}EST. {compact(wall.dollar)}</small></>:"—"}</td>)}</tr>})}</tbody></table></div><footer>Strike values match the graph: each phase anchor is the median of its first five minutes. Strength remains live and is recalculated every 5 seconds; estimated GEX uses delayed OI × Greeks.</footer></section>;
}
