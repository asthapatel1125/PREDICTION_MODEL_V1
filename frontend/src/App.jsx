...
function SystemScorecardRow({system,label,calls,state}){
  ...
  return <tr className={`system-scorecard-row ${scorecardClass}`}><td className="scorecard-system"><b>{label}</b></td><td className="scorecard-success">{counts.succeeded}</td><td className="scorecard-failure">{counts.failed}</td><td className="scorecard-success">{counts.succeeding}</td><td className="scorecard-failure">{counts.failing}</td><td className="scorecard-favorable-cell">{averageFavorable===null?"—":`+${averageFavorable.toFixed(2)} PT`}</td><td className="scorecard-targets">{speedBuckets.map(bucket=><span key={bucket.label}><b>{bucket.count}</b><small>{bucket.label}</small></span>)}</td></tr>;
}
