  ...
  return <ChartShell expanded={expanded} setExpanded={setExpanded} className={`greek-order-chart order-${order}`}>
    ...
    <div className="greek-chart-stage" onWheel={event=>{event.preventDefault();const anchor=chartCursor(event,dims,rows.length,1);setCursor({...anchor,value:order==="first"?anchor.value:scale.unproject(anchor.value*scale.projectedMax)});if(event.shiftKey)viewport.move(event.deltaY>0?10:-10);else zoomChart(event.deltaY<0?.5:-.5,anchor.index)}} onPointerDown={onPointerDown} onPointerMove={onPointerMove} onPointerUp={onPointerUp} onPointerCancel={()=>{drag.current=null}} onPointerLeave={()=>{drag.current=null;setCursor(null)}}>
      <svg viewBox={`0 0 ${dims.width} ${dims.height}`} role="img" aria-label={`${config.label} live options Greeks over time with signed exposure on the y axis`}>
        ...
        {[0,1,2,3,4].map(tick=>{const projected=1-tick/2,value=scale.unproject(projected*scale.projectedMax),yy=yProjected(projected),label=order==="first"?`${projected>0?"+":""}${Math.round(projected*100)}%`:formatValue(value);return <g key={`gy-${tick}`}><line className="greek-grid" x1={dims.left} x2={dims.width-dims.right} y1={yy} y2={yy}/><text className="greek-axis" x={dims.left-10} y={yy+4} textAnchor="end">{label}</text></g>})}
        ...
        <line className="greek-zero" x1={dims.left} x2={dims.width-dims.right} y1={yProjected(0)} y2={yProjected(0)}/>
        ...
        {cursor&&<g><line className="greek-crosshair" x1={x(cursor.index)} x2={x(cursor.index)} y1={dims.top} y2={dims.height-dims.bottom}/><line className="greek-crosshair horizontal" x1={dims.left} x2={dims.width-dims.right} y1={order==="first"?yProjected(cursor.value):y(cursor.value)} y2={order==="first"?yProjected(cursor.value):y(cursor.value)}/>{config.series.map(([name,color])=>{const value=seriesValue(hovered,name);return Number.isFinite(value)?<circle key={name} className="greek-hover-dot" cx={x(cursor.index)} cy={y(value,name)} r="4" style={{fill:color}}/>:null})}</g>} 
      </svg>
      <ChartCoordinateTooltip {...{cursor,row:hovered,series:config.series,formatTime,formatValue,seriesValue}} cursorValueText={order==="first"?value=>`Relative scale ${value>0?"+":""}${Math.round(value*100)}%`:undefined}/>
