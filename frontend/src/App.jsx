...
function SystemScorecardRow({system,label,calls,state}){
  ...
  return <tr className={`system-scorecard-row ${scorecardClass}`}><td className="scorecard-system"><b>{label}</b></td><td className="scorecard-succeeded">{counts.succeeded}</td><td className="scorecard-failed">{counts.failed}</td><td className="scorecard-succeeding">{counts.succeeding}</td><td className="scorecard-failing">{counts.failing}</td><td className={`scorecard-direction ${direction==="UP"?"up":direction==="DOWN"?"down":""}`}>{direction}</td><td className="scorecard-streak">{winStreak?`${winStreak} WIN${winStreak===1?"":"S"}`:"—"}</td><td className="scorecard-targets">{speedBuckets.map(bucket=><span key={bucket.label}><b>{bucket.count}</b><small>{bucket.label}</small></span>)}</td></tr>;
}
