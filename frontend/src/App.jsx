...
function GammaDynamicsV2FlowLog({history=[],state,symbol}){
  ...
  const rows=[...new Map(source.map(row=>[row.timestamp,row])).values()].sort((a,b)=>new Date(b.timestamp)-new Date(a.timestamp)).map(row=>({timestamp:row.timestamp,metrics:row.gamma_dynamics_v2?.chain_metrics??{}})).filter(row=>Number(row.metrics.chain_available)>0).slice(0,90);
...
function GammaEventGraphModal({event,call,version=1,onClose}){
  ...
  return createPortal(<div className="event-graph-backdrop" role="dialog" aria-modal="true" aria-label="Gamma Dynamics event details" onPointerDown={pointerEvent=>{if(pointerEvent.target===pointerEvent.currentTarget)onClose()}}><section className="event-graph-dialog event-call-dialog"><header><div><span>GAMMA DYNAMICS {version}.0 · EVENT DETAILS</span><b>{call.symbol} · {biasLabel(call.direction)} · {logDate(call.alerted_at)} {logTime(call.alerted_at)}</b></div><button type="button" onClick={onClose}>CLOSE</button></header><div className="event-call-dialog-body"><GammaEventLogSummary event={event} call={call}/><FiftyPointOutcomeCard call={call} system={version===2?"GAMMA_DYNAMICS_V2":"GAMMA_DYNAMICS"}/></div></section></div>,document.body);
}
