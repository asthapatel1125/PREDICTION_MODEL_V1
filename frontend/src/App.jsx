...
function GammaDynamicsLog({history,state,symbol,calls=[]}){
  ...
  return <article className="panel gamma-dynamics-log">
    ...
    <div className="gamma-log-scroll-shell">
      ...
      <thead><tr><th>DIRECTION</th><th>CALL STATE</th><th>DATE · EASTERN</th><th>TIME · MS</th><th>MARKET HOUR</th><th>SOURCE</th><th>DATUM / ALERT PRICE</th><th>SESSION TARGET</th><th>STRIKE / REFERENCE</th><th>DYNAMIC / EXTREME HIGH</th><th>HIGH CHANGE</th><th>TIME TO HIGH</th><th>DYNAMIC / EXTREME LOW</th><th>LOW CHANGE</th><th>TIME TO LOW</th><th>CURRENT / FINAL</th><th>CURRENT / FINAL CHANGE</th><th>STRONGEST GREEK</th><th>WEAKEST GREEK</th><th>INTENSITY</th><th>PRESSURE</th>{GAMMA_DYNAMICS_GREEKS.flatMap(name=>[<th key={`${name}-strike`}>{name.toUpperCase()} @ STRIKE · RAW</th>,<th key={`${name}-normalized`}>{name.toUpperCase()} @ STRIKE · NORM</th>,<th key={`${name}-high`}>{name.toUpperCase()} HIGH</th>,<th key={`${name}-low`}>{name.toUpperCase()} LOW</th>])}{["STRIKE","FINAL"].flatMap(phase=>GAMMA_RANKS.map(rank=><th className={`greek-rank-head ${rank}`} key={`${phase}-${rank}`}>{phase} · {rank.toUpperCase()}</th>))}<th>EVENT ID</th></tr></thead>
