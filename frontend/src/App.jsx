...
function TimePartsPicker({label,value,onChange}){
  const [rawHour="",rawMinute="00"]=String(value??"").split(":"),hour24=Number(rawHour),hasValue=Boolean(value),period=hasValue?(hour24>=12?"PM":"AM"):"",hour12=hasValue?String(hour24%12||12).padStart(2,"0"):"",minute=hasValue?rawMinute:"";
  const commit=(part,next)=>{const draft={hour:hour12||"12",minute:minute||"00",period:period||"AM",[part]:next};let converted=Number(draft.hour)%12;if(draft.period==="PM")converted+=12;onChange(`${String(converted).padStart(2,"0")}:${draft.minute}`)};
  return <div className="time-range-row"><span>{label}</span><div className="time-parts-picker"><select aria-label={`${label} hour`} value={hour12} onChange={event=>commit("hour",event.target.value)}><option value="" disabled>HH</option>{TIME_HOURS.map(option=><option value={option} key={option}>{option}</option>)}</select><b>:</b><select aria-label={`${label} minute`} value={minute} onChange={event=>commit("minute",event.target.value)}><option value="" disabled>MM</option>{TIME_MINUTES.map(option=><option value={option} key={option}>{option}</option>)}</select><select className="period" aria-label={`${label} AM or PM`} value={period} onChange={event=>commit("period",event.target.value)}><option value="" disabled>AM/PM</option><option value="AM">AM</option><option value="PM">PM</option></select>{hasValue&&<button type="button" className="clear-time" onClick={()=>onChange("")} aria-label={`Clear ${label.toLowerCase()} time`}>×</button>}</div></div>;
}
function EventLogTimeRangeFilter({fromTime,toTime,onApply}){
  const [draftFrom,setDraftFrom]=useState(fromTime),[draftTo,setDraftTo]=useState(toTime),detailsRef=useRef(null);
  useEffect(()=>{setDraftFrom(fromTime);setDraftTo(toTime)},[fromTime,toTime]);
  useEffect(()=>{const close=event=>{const details=detailsRef.current;if(details?.open&&!details.contains(event.target)){setDraftFrom(fromTime);setDraftTo(toTime);details.removeAttribute("open")}};const escape=event=>{if(event.key==="Escape"){setDraftFrom(fromTime);setDraftTo(toTime);detailsRef.current?.removeAttribute("open")}};document.addEventListener("pointerdown",close);document.addEventListener("keydown",escape);return()=>{document.removeEventListener("pointerdown",close);document.removeEventListener("keydown",escape)}},[fromTime,toTime]);
  const display=value=>{if(!value)return "";const [hour,minute]=value.split(":"),numeric=Number(hour);return `${String(numeric%12||12).padStart(2,"0")}:${minute} ${numeric>=12?"PM":"AM"}`};
  const apply=()=>{onApply(draftFrom,draftTo);detailsRef.current?.removeAttribute("open")};
  return <div className="event-time-range"><span>TIME RANGE</span><details ref={detailsRef}><summary>{fromTime||toTime?`${display(fromTime)||"START"} – ${display(toTime)||"END"}`:"All times"}</summary><div className="time-range-menu"><TimePartsPicker label="FROM" value={draftFrom} onChange={setDraftFrom}/><TimePartsPicker label="TO" value={draftTo} onChange={setDraftTo}/><div className="time-range-actions"><button type="button" onClick={()=>{setDraftFrom("");setDraftTo("")}}>CLEAR</button><button type="button" className="apply" onClick={apply}>APPLY</button></div></div></details></div>;
}
...
function DeltaDynamicsEventLog({history,state,symbol,calls=[]}){
  ...
  return <article className="panel gamma-dynamics-log delta-dynamics-log"><header className="panel-head"><div><span>DELTA DYNAMICS EVENT LOG</span><h2>Every qualified call · observed minute high/low path</h2></div><div className="gamma-log-actions"><button type="button" className="copy-table" onClick={copy} disabled={!visible.length}>{copied?"✓ COPIED":"COPY SUMMARIES"}</button><b>{visible.length} EVENTS</b></div></header>
    <div className="gamma-log-controls"><label>SORT<select value={sortOrder} onChange={event=>setSortOrder(event.target.value)}><option value="newest">Newest first</option><option value="oldest">Oldest first</option></select></label><label>DATE<input type="date" value={filterDate} onChange={event=>setFilterDate(event.target.value)}/></label><EventLogTimeRangeFilter fromTime={fromTime} toTime={toTime} onApply={(from,to)=>{setFromTime(from);setToTime(to)}}/><button type="button" onClick={()=>{setFilterDate("");setFromTime("");setToTime("")}}>CLEAR FILTER</button></div>
...
function GammaDynamicsLog({history,state,symbol,calls=[]}){
  ...
  const legMetrics=(call,leg)=>{
    ...
    const datum=number(leg.datum),price=Number.isFinite(Number(leg.final_price))?number(leg.final_price):number(call.current_price??call.final_price??call.minute_bars?.at(-1)?.close,datum);
  ...
  return <article className="panel gamma-dynamics-log">
    ...
    <div className="gamma-log-controls">
      ...
      <EventLogTimeRangeFilter fromTime={fromTime} toTime={toTime} onApply={(from,to)=>{setFromTime(from);setToTime(to)}}/>
    ...
    <div className="gamma-log-scroll-shell">
      ...
      <thead><tr><th>DIRECTION</th><th>CALL STATE</th><th>DATE · EASTERN</th><th>TIME · MS</th><th>MARKET HOUR</th><th>SOURCE</th><th>DATUM / ALERT PRICE</th><th>SESSION TARGET</th><th>STRIKE / REFERENCE</th><th>DYNAMIC / EXTREME HIGH</th><th>HIGH CHANGE</th><th>TIME TO HIGH</th><th>DYNAMIC / EXTREME LOW</th><th>LOW CHANGE</th><th>TIME TO LOW</th><th>CURRENT / FINAL</th><th>CURRENT / FINAL CHANGE</th><th>STRONGEST GREEK</th><th>WEAKEST GREEK</th><th>INTENSITY</th><th>PRESSURE</th>{GAMMA_DYNAMICS_GREEKS.flatMap(name=>[<th key={`${name}-strike`}>{name.toUpperCase()} @ STRIKE</th>,<th key={`${name}-high`}>{name.toUpperCase()} HIGH</th>,<th key={`${name}-low`}>{name.toUpperCase()} LOW</th>])}<th>EVENT ID</th></tr></thead>
      <tbody>{visibleEvents.map((event,index)=>{
        ...
        return <Fragment key={`${event.timestamp}-${event.decision}-${index}`}>
          <tr className={`gamma-call-${view.key}`}><td><span className={`direction-pill ${event.decision.toLowerCase()}`}>{event.decision==="UP"?"UPWARD":"DOWNWARD"}</span></td><td><span className={`gamma-call-state ${view.key}`}>{view.label}</span></td><td>{logDate(event.timestamp)}</td><td>{logTime(event.timestamp)}</td><td><span className="market-hour">{marketHourLabel(event.timestamp)}</span></td><td>{event.symbol}</td><td className="extreme-price">{Number.isFinite(datum)?datum.toFixed(4):"—"}</td><td className="extreme-price">{Number.isFinite(datum)?number(call?.target_price,reachFromDatum(datum,event.decision,event.timestamp)).toFixed(4):"—"}</td><td className="extreme-price">{Number.isFinite(strike.value)?`${strike.estimated?"REF ":""}${strike.value.toFixed(2)}`:"—"}</td><td className="extreme-price">{priceValue(call,high)}</td><td>{pointValue(call,high)}</td><td className="extreme-time">{extremeTime(call,call?.seconds_to_high,finished)}</td><td className="extreme-price">{priceValue(call,low)}</td><td>{pointValue(call,low)}</td><td className="extreme-time">{extremeTime(call,call?.seconds_to_low,finished)}</td><td className="extreme-price">{priceValue(call,livePrice)}</td><td>{pointValue(call,livePrice)}</td><td><GreekAuditBadge label={call?.strongest_greek_current??call?.strongest_greek}/></td><td><GreekAuditBadge label={call?.weakest_greek_current??call?.weakest_greek}/></td><td>{Number.isFinite(event.intensity)?pct(event.intensity):"—"}</td><td>{Number.isFinite(event.pressure)?`${event.pressure>0?"+":""}${event.pressure.toFixed(2)}`:"—"}</td>{GAMMA_DYNAMICS_GREEKS.flatMap(name=>{const strikeValue=call?.greek_values_at_signal?.[name]??event[name];return [<td key={`${name}-strike`}>{signedGreek(strikeValue)}</td>,<td className="greek-extreme-high" key={`${name}-high`}>{signedGreek(call?.greek_values_highest?.[name]??strikeValue)}</td>,<td className="greek-extreme-low" key={`${name}-low`}>{signedGreek(call?.greek_values_lowest?.[name]??strikeValue)}</td>]})}<td><button type="button" className="call-id" onClick={()=>navigator.clipboard.writeText(event.id)} title="Copy event ID">{event.id}</button></td></tr>
