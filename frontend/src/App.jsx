...
function WallIntelligenceRefined({symbol}){
  const [spectrum,setSpectrum]=useState([]),[breaks,setBreaks]=useState([]),[history,setHistory]=useState([]),[frame,setFrame]=useState("15M"),[category,setCategory]=useState("Walls & Levels"),[variable,setVariable]=useState("Call Wall"),[traces,setTraces]=useState(["Call Wall"]);
  useEffect(()=>{const controller=new AbortController();const refresh=()=>Promise.all([fetchWallSpectrum(symbol,controller.signal),fetchWallBreaks(symbol,controller.signal),fetchWallSummaryHistory(symbol,controller.signal)]).then(([s,b,h])=>{setSpectrum(s.rows??[]);setBreaks(b.rows??[]);setHistory(h.rows??[])}).catch(()=>{});refresh();const timer=setInterval(refresh,5000);return()=>{controller.abort();clearInterval(timer)}},[symbol]);
  const latest=spectrum.at(-1)||{},walls=latest.walls||{},count={"5M":60,"15M":180,"1H":720,"ALL":720}[frame],rows=spectrum.slice(-count),compact=v=>{const n=number(v);return Math.abs(n)>=1e9?`${(n/1e9).toFixed(2)}B`:Math.abs(n)>=1e6?`${(n/1e6).toFixed(1)}M`:n.toFixed(2)},dist=(level)=>{const points=number(level)-number(latest.spot);const pct=number(latest.spot)?points/number(latest.spot)*100:0;return `${points>=0?"+":""}${points.toFixed(2)} pts (${points>=0?"+":""}${pct.toFixed(2)}%)`},band=pct=>Math.abs(pct)<.2?"AT WALL / PINNED":Math.abs(pct)<.5?"TIGHT":Math.abs(pct)<1.5?"NEUTRAL":"EXTENDED";
  const call=walls.CALL_WALL||{},put=walls.PUT_WALL||{},support=walls.SUPPORT||{},resistance=walls.RESISTANCE||{},zero=walls.ZERO_GAMMA||{};
  const spread=number(resistance.strike)-number(support.strike),spreadPct=number(latest.spot)?spread/number(latest.spot)*100:0;
