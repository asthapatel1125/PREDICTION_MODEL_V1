...
function SystemScorecardRow({system,label,calls,state}){
  ...
  const recentCompleted=[...completed].sort((left,right)=>new Date(right.target_reached_at??right.final_price_at??right.expired_at??right.alerted_at??0)-new Date(left.target_reached_at??left.final_price_at??left.expired_at??left.alerted_at??0));
  const winBreak=recentCompleted.findIndex(call=>scorecardCallState(call)!=="succeeded"),winStreak=recentCompleted.length?(winBreak<0?recentCompleted.length:winBreak):0;
  return <tr className={`system-scorecard-row ${scorecardClass}`}><td className="scorecard-system"><b>{label}</b></td><td className="scorecard-success">{counts.succeeded}</td><td className="scorecard-failure">{counts.failed}</td><td className="scorecard-success">{counts.succeeding}</td><td className="scorecard-failure">{counts.failing}</td><td className={`scorecard-direction ${direction==="UP"?"up":direction==="DOWN"?"down":""}`}>{direction}</td><td className="scorecard-streak">{winStreak?`${winStreak} WIN${winStreak===1?"":"S"}`:"—"}</td><td className="scorecard-favorable-cell">{averageFavorable===null?"—":`+${averageFavorable.toFixed(2)} PT`}</td><td className="scorecard-targets">{speedBuckets.map(bucket=><span key={bucket.label}><b>{bucket.count}</b><small>{bucket.label}</small></span>)}</td></tr>;
}
