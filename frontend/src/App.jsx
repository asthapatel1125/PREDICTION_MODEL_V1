...
function CustomGreekChart({chart,index,history,state,symbol,onChange,onRemove,canRemove}){
  const [intervalSeconds,setIntervalSeconds]=useState(5),[expanded,setExpanded]=useState(false),[hoverIndex,setHoverIndex]=useState(null);
  const drag=useRef(null),viewport=useGreekViewport(history,state,symbol,intervalSeconds,expanded?160:96),rows=viewport.visible;
  ...
  const formatValue=value=>{const magnitude=Math.abs(number(value));return magnitude>0&&magnitude<.001?number(value).toExponential(2):number(value).toFixed(4)};
  const formatTime=value=>value?new Date(value).toLocaleTimeString([],{hour:"2-digit",minute:"2-digit",second:"2-digit"}):"—",hovered=rows[hoverIndex??rows.length-1];
  const onPointerMove=event=>{const bounds=event.currentTarget.getBoundingClientRect(),point=Math.round((event.clientX-bounds.left)/bounds.width*(rows.length-1));setHoverIndex(Math.max(0,Math.min(rows.length-1,point)));if(drag.current){const delta=event.clientX-drag.current.x;if(Math.abs(delta)>3)drag.current.moved=true;viewport.setOffset(Math.max(0,Math.min(viewport.maxOffset,drag.current.offset-Math.round(delta/7))))}};
