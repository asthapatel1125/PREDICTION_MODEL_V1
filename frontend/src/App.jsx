...
function GammaDynamicsChart({history=[],state,symbol,deltaMode=false,gammaVersion=1}){
  ...
  const copyChartGreeks=async()=>{
    ...
    const values=rows.map(row=>[logDate(row.timestamp),logTime(row.timestamp),row.symbol??symbol,...series.flatMap(([name])=>{const raw=rawValueFor(row,name),normalized=valueFor(row,name);return [Number.isFinite(raw)?raw:"",Number.isFinite(normalized)?normalized:""]})]);
  ...
  return <ChartShell expanded={expanded} setExpanded={setExpanded} className={`gamma-dynamics-history-chart gamma-dynamics-chart ${deltaMode?"delta-dynamics-chart":v2?"gamma-dynamics-v2-chart":""}`}>
    <div className="greek-chart-header"><div><span>{deltaMode?"DELTA DYNAMICS HISTORY":`GAMMA DYNAMICS ${gammaVersion}.0 HISTORY · NORMALIZED LIVE STREAM`}</span><h2>{symbol} · {deltaMode?"Ultima / Zomma / Gamma / Speed / Color / Delta":series.map(item=>pretty(item[0])).join(" / ")}</h2></div><div className="greek-header-actions">{deltaMode&&<button type="button" className="copy-table delta-minute-stream-button" onClick={copyMinuteStream} disabled={!minuteStream.length}>{streamCopied?"✓ COPIED":"COPY MINUTE STREAM"}</button>}<button type="button" className="copy-table" onClick={copyChartGreeks} disabled={!rows.length}>{greeksCopied?"✓ COPIED":"COPY GREEKS"}</button><ChartTimeControls {...{intervalSeconds,setIntervalSeconds,isLive:viewport.isLive,setOffset:viewport.setOffset,expanded,setExpanded,zoom,onZoom:zoomChart}}/></div></div>
