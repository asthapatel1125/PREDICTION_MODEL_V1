...
export const fetchWallPriceSeries = (symbol, bucketSeconds, signal) =>
  request(`/api/v1/walls/price-series?symbol=${encodeURIComponent(symbol)}&bucket_seconds=${encodeURIComponent(bucketSeconds)}`, { signal });
const wallExposureRequests=new Map();
export function fetchWallExposureSeries(symbol,bucketSeconds,signal){
  const key=`${String(symbol).toUpperCase()}:${bucketSeconds}`;
  if(!wallExposureRequests.has(key)){
    const pending=request(`/api/v1/walls/exposure-series?symbol=${encodeURIComponent(symbol)}&bucket_seconds=${encodeURIComponent(bucketSeconds)}`)
      .finally(()=>wallExposureRequests.delete(key));
    wallExposureRequests.set(key,pending);
  }
  return wallExposureRequests.get(key).then(result=>{if(signal?.aborted)throw new DOMException("Aborted","AbortError");return result});
}
