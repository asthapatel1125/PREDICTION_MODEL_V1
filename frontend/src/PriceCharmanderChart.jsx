...
export default function PricePhoenixChart({ rows = [], symbol = "QQQ", qqqPointsPer50Nq = 1.235 }) {
  ...
  const sp500Estimate=symbol.toUpperCase()==="SPY"&&Number.isFinite(hoveredPrice)?hoveredPrice*10:null;
  ...
  return <section className="price-charmander">
    ...
    <div className="price-charmander-body">
      ...
      <div className="price-charmander-plot-stack">
       <input className="price-charmander-top-scroll" type="range" min="0" max={Math.max(0,size.width-viewportWidth)} value={Math.min(scrollOffset,Math.max(0,size.width-viewportWidth))} onChange={event=>{const left=Number(event.currentTarget.value);if(frameRef.current)frameRef.current.scrollLeft=left;setScrollOffset(left)}} aria-label="Scroll Phoenix chart history"/>
       <div className="price-charmander-frame" ref={frameRef} onScroll={event=>{const node=event.currentTarget;setScrollOffset(node.scrollLeft);setViewportWidth(node.clientWidth);followingLiveRef.current=node.scrollLeft>=node.scrollWidth-node.clientWidth-18;if(node.scrollLeft<80)loadEarlier()}} onPointerMove={pointerMove} onPointerLeave={() => setHover(null)} onDoubleClick={resetView}>
        ...
        {hoveredIndex != null && <aside style={{width:`${Math.max(320,viewportWidth-24)}px`}}><b>{hover===null?"LIVE · ":""}{timeLabel(calculated.timestamps[hoveredIndex])} ET</b><span>{symbol} <strong>{hoveredPrice?.toFixed(2)}</strong></span><span>CONSENSUS <strong>{hoveredConsensus >= 0 ? "+" : ""}{hoveredConsensus?.toFixed(3)}</strong></span>{nqEstimate!==null&&<span className="phoenix-index-conversion" title={`Estimated NQ equivalent: QQQ × 50 ÷ ${Math.max(Number(qqqPointsPer50Nq)||1.235,.0001).toFixed(3)}. Proxy only; not a live CME quote.`}>NQ EST <strong>{nqEstimate.toFixed(2)}</strong></span>}{sp500Estimate!==null&&<span className="phoenix-index-conversion" title="Estimated S&P 500 index equivalent: SPY × 10. Proxy only; not an official index quote.">S&amp;P 500 EST <strong>{sp500Estimate.toFixed(2)}</strong></span>}</aside>}
       </div>
      </div>
    </div>
